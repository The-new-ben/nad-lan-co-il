...
import json, os, re, shutil, html, csv, io
import build as B
...
def broker_bundle(eds):
    ...
    subprocess.run(['python3', B.B + 'broker/build_broker.py'], check=True, capture_output=True)
    for lang in ('he', 'en'):
    ...
    shutil.copytree(B.BASE + 'canvas/project', d + 'claude-design-canvas/project', dirs_exist_ok=True)
    if os.path.isdir(B.BASE + 'minisite/project'):
        shutil.copytree(B.BASE + 'minisite/project', d + 'claude-design-minisite/project', dirs_exist_ok=True)
    ...
    shutil.copytree(B.B + 'broker', src + 'broker', dirs_exist_ok=True, ignore=shutil.ignore_patterns('__pycache__', 'v1'))
    shutil.copytree(B.B + 'design', src + 'design', dirs_exist_ok=True, ignore=shutil.ignore_patterns('__pycache__'))
...
def main():
    ...
    shutil.copytree(B.B + 'scripts', PKG + 'scripts')
    src = PKG + 'source/'
    ...
    for d in ('templates', 'assets', 'editorial', 'scripts', 'docs'):
        shutil.copytree(B.B + d, src + 'build/' + d)
    shutil.copytree(B.BASE + 'dossiers', src + 'dossiers')
