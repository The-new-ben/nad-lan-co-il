...
T = {
 'he': dict(toc='תוכן העמוד', rail='מחיר ויצירת קשר', sec_home='הבית', sec_building='הבניין', sec_location='הסביבה',
   ...
   sec_numbers='המספרים', sec_market='השוק', sec_checks='לפני החלטה', sec_faq='שאלות ותשובות', faq_h2='שאלות שכדאי לשאול', sec_broker='המתווכת בבלעדיות',
   sec_sources='מקורות, בסיס הנתונים והערות', distances='מרחקים', schools='חינוך', transport='תחבורה',
   tag_broker='מיטל קציר', tag_calc='חישוב', tag_src='מקור', tag_tx='עסקה', tag_ask='מבוקש', tag_rep='דווח',
   ...
   legend_broker='נתון מאת מיטל קציר, המתווכת בבלעדיות', legend_calc='חישוב של nad-lan על בסיס נתונים מצוטטים', legend_src='מספר המקור ברשימה',
...
def build_listing(ed, dos, lang):
    ...
    L['plate'] = {'name': L_(ed['plate_name'], lang),
                  'caption': '' if lang == 'he' else 'Property photography to come from the listing broker. The drawing does not depict the property.',
    ...
    L['facts_note'] = (tagh('broker', lang) + ' ' + ('פרטי הנכס בשורה זו מאת מיטל קציר, מתווכת מורשית, רישיון 3131540' if lang == 'he' else 'All figures in this row were provided by the listing broker')) if all_broker else ''
    # toc
    ...
    L['agent'] = {'mono': 'מ' if lang == 'he' else 'M', 'name': f"{BROKER['name'][lang]}", 
                  'line': (f"{BROKER['brand'][lang]} · {lic} · " + (f"בלעדיות על הנכס. העמוד הוכן על ידי nad-lan.co.il בשיתוף מיטל קציר." if lang == 'he' else "Exclusive listing. Page prepared by nad-lan.co.il in cooperation with the broker.")),
    ...
    posts = ed.get('posts', [])
    if lang == 'he':
        L['disclaimer'] = ('המידע בעמוד נאסף ונערך על ידי nad-lan.co.il מפרסומי מיטל קציר וממקורות ציבוריים, והוא נועד להתמצאות בלבד. אין בו הצעה מחייבת, שמאות, ייעוץ משפטי, ייעוץ מס או ייעוץ השקעות. '
            'חישובי מס, מימון, ארנונה ותשואה הם המחשה לפי הכללים שפורסמו ביום העדכון ועשויים להשתנות. לאמת כל נתון מול המסמכים הרשמיים, עורך דין, שמאי ויועץ מס לפני קבלת החלטה. '
...
def build_market(ed, dos, lang, book):
    ...
    if sidx:
        ...
        M['tables'].append({'title': L_(mk.get('sale_title'), lang) or ('עסקאות ומודעות מכירה להשוואה' if he else 'Sales and asking prices for comparison'), 'head': t['comp_head_sale'], 'rows': rows,
                            'caption': ('עסקאות מוצגות לפני מודעות. מחיר למ״ר מחושב על השטח שפורסם. לאמת עסקאות באתר נדל״ן של רשות המסים. אין זו שמאות.' if he else 'Recorded sales are listed before asking prices. Price per sqm is computed on the published area. Verify sales on the Tax Authority real estate site. This is not an appraisal.')})
