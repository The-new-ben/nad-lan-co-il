> **RECONCILIATION NOTE — added by Claude Code, 2026-07-01, do not delete on re-edit.**
> This document was authored by an Antigravity (Gemini) session and pasted into a Claude
> Code session for archival. It originally lived only at a local Windows path
> (`C:\Users\pro\.gemini\antigravity\brain\...`) that no other agent's environment can
> read — per `skills/skill-release-discipline-and-mistakes.md` M9 ("if it's not committed
> to the repo, it does not exist"), it is committed here verbatim so it survives.
>
> Two corrections a researching agent needs before acting on it:
> 1. **"Parent theme, do not edit directly" is not the current practice.** `nadlan-revenue`
>    (this repo's root — `style.css`, `functions.php`, `patterns/`, `templates/`) is edited
>    directly and constantly; that is the live, working UPress-git-synced pipeline. Verified
>    2026-07-01: `curl https://nad-lan.co.il/` shows **both**
>    `wp-content/themes/nadlan-revenue` and `wp-content/themes/nadlan-platform-child` loaded
>    — the child theme (`themes/nadlan-platform-child/`) is live now, not staging-only.
> 2. **Section 7's "stop injecting CSS into the plugin" directly contradicts a standing
>    decision** in `handoff/external-agent-packages/2026-06-28/REVIEW-AND-SOLUTION.md`:
>    "We do NOT add a third/fourth engine... finish the engine [inside `nadlan-config`]
>    ... one clean release at a time." That decision was made specifically to stop two
>    competing presentation systems from stacking. The plugin-CSS-injection pattern this
>    document calls "bad practice" (e.g. `nadlan_config_emergency_css()`, shipped through
>    v1.69.68) exists *because* the child theme has no live-sync deploy path (see the
>    UPress single-git-target constraint below) — it is a symptom of the exact structural
>    problem Section 5 correctly diagnoses, not an agent ignoring the diagnosis. The fix is
>    to give the child theme (and/or the plugin) a real deploy path, not to freeze
>    mid-migration with emergency fixes stuck in the wrong layer.
>
> External research on solving the deploy-pipeline problem this document raises (Section 5)
> is tracked separately in `docs/research/` — see `docs/research/2026-07-01-ai-wordpress-direct-access-tools.md`
> (REST/MCP/Adminer angle) and the follow-up multi-source research this document triggered.
>
> The content below this line is unedited from the original Antigravity handoff.
...
> **Note added by Claude, 2026-07-01:** the paths in this section are local to one
> Windows machine and are unreachable from any other agent's environment (Claude Code
> web/CLI sessions, Codex sandboxes, Cowork). Only `plugins/nadlan-config/inc/showroom-engine.php`
> resolves (it is in this repo, relative path `plugins/nadlan-config/inc/showroom-engine.php`).
> The `project_state_summary.md` / `architecture_and_design_flow.md` / etc. strategic
> artifacts and the Lovable prototype source referenced above are **not yet in this repo**
> — if they should be durable project knowledge, they need to be pasted/committed the same
> way this document was, e.g. under `handoff/antigravity/2026-07-01-mega-master-handoff/`
> alongside this file, or `handoff/lovable/2026-06-23-war-room-sync/` already holds an
> earlier Lovable prototype export that may be the same or a prior version of what's
> referenced here — worth diffing before re-importing.
