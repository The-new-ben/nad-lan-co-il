<?php
/**
 * nadlan-config - reliability health endpoint + bounded event log (v1.55.0).
 */
...
if ( ! function_exists( 'nadlan_health_probe_http' ) ) {
	function nadlan_health_probe_http( $name, $url ) {
		...
		$res = wp_remote_get( $url, array(
			...
			'sslverify'           => true,
			'user-agent'          => 'NadLanHealth/1.55.0; ' . home_url( '/' ),
		) );
...
if ( ! function_exists( 'nadlan_reliability_ping_heartbeat' ) ) {
	function nadlan_reliability_ping_heartbeat( $name ) {
		...
		wp_remote_get( $url, array(
			...
			'blocking'    => false,
			'sslverify'   => true,
			'user-agent'  => 'NadLanCronHeartbeat/1.55.0; ' . home_url( '/' ),
		) );
