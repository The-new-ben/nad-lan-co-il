...
if ( ! function_exists( 'nadlan_utopia_article_manifest' ) ) {
	function nadlan_utopia_article_manifest() {
		return array(
			'he' => array(
				'sha256' => '334fbb6727f087091cfb9bbef304ff2e000b3f6d24d3bd58e0203ca337631644',
				'locale' => 'he-IL',
				'dir' => 'rtl',
			...
			'ru' => array(
				'sha256' => '056a3d24e970691db7ed5142759ea9876bbdd666b61ac04e2173f5dfcd5f1459',
				'locale' => 'ru-RU',
				'dir' => 'ltr',
			...
			'ar' => array(
				'sha256' => '70b565854ff0c8710923e95280481c5a81c504806f546fd36c20a9557f4168e2',
				'locale' => 'ar',
				'dir' => 'rtl',
...
if ( ! function_exists( 'nadlan_utopia_sample_plans' ) ) {
	function nadlan_utopia_sample_plans( $lang ) {
		...
		return array(
			array( 'building' => 'n1', 'label' => $p . ' N1 4A', 'url' => 'https://utopiatlv.co.il/wp-content/uploads/2025/07/UTOPIA-N1-4A-21222-copy.pdf' ),
			array( 'building' => 'n1', 'label' => $p . ' N1 5G', 'url' => 'https://utopiatlv.co.il/wp-content/uploads/2025/07/UTOPIA-N1-5G-62-copy.pdf' ),
			array( 'building' => 'n1', 'label' => $p . ' N1 5E', 'url' => 'https://utopiatlv.co.il/wp-content/uploads/2025/07/UTOPIA-N1-5E-404448525660-copy.pdf' ),
			array( 'building' => 'n1', 'label' => $p . ' N1 3A', 'url' => 'https://utopiatlv.co.il/wp-content/uploads/2025/07/UTOPIA-N1-3A-81828-copy.pdf' ),
			array( 'building' => 's1', 'label' => $p . ' S1 5G', 'url' => 'https://utopiatlv.co.il/wp-content/uploads/2025/07/5G-S1-204.pdf' ),
			array( 'building' => 's1', 'label' => $p . ' S1 3P', 'url' => 'https://utopiatlv.co.il/wp-content/uploads/2025/07/3P-S18094108122136150164.pdf' ),
			array( 'building' => 's1', 'label' => $p . ' S1 4D', 'url' => 'https://utopiatlv.co.il/wp-content/uploads/2025/07/4D-S156708498112126140154.pdf' ),
		);
	}
}
...
if ( ! function_exists( 'nadlan_utopia_acquire_release_lock' ) ) {
	function nadlan_utopia_acquire_release_lock() {
		$key   = 'nadlan_utopia_release_v172128_lock';
...
if ( ! function_exists( 'nadlan_utopia_release_complete' ) ) {
	function nadlan_utopia_release_complete() {
		return (string) nadlan_utopia_db_option( 'nadlan_utopia_release_v172128', '' ) === '1';
	}
}
...
if ( ! function_exists( 'nadlan_utopia_read_run' ) ) {
	function nadlan_utopia_read_run() {
		$run = get_option( 'nadlan_utopia_release_v172128_run', false );
...
if ( ! function_exists( 'nadlan_utopia_store_run' ) ) {
	function nadlan_utopia_store_run( $run, $create = false ) {
		...
		$stored = $create
			? add_option( 'nadlan_utopia_release_v172128_run', $run, '', 'no' )
			: update_option( 'nadlan_utopia_release_v172128_run', $run, false );
		if ( ! $stored && get_option( 'nadlan_utopia_release_v172128_run', false ) !== $run ) {
...
if ( ! function_exists( 'nadlan_utopia_read_backup' ) ) {
	function nadlan_utopia_read_backup() {
		$backup = get_option( 'nadlan_utopia_backup_v172128', false );
...
if ( ! function_exists( 'nadlan_utopia_store_backup' ) ) {
	function nadlan_utopia_store_backup( $backup ) {
		...
		$changed = update_option( 'nadlan_utopia_backup_v172128', $backup, false );
		if ( ! $changed ) {
			$current = get_option( 'nadlan_utopia_backup_v172128', false );
			if ( $current !== $backup ) {
...
if ( ! function_exists( 'nadlan_utopia_backup_state' ) ) {
	function nadlan_utopia_backup_state( $posts ) {
		if ( get_option( 'nadlan_utopia_backup_v172128', false ) !== false ) {
		...
		$backup['checksum'] = nadlan_utopia_backup_checksum( $backup );
		if ( ! add_option( 'nadlan_utopia_backup_v172128', $backup, '', 'no' ) ) {
...
if ( ! function_exists( 'nadlan_utopia_restore_backup' ) ) {
	function nadlan_utopia_restore_backup( $extra_resources = array(), $operator_hold = false, $run_token = '' ) {
		...
		if ( is_wp_error( $backup ) ) {
			update_option( 'nadlan_utopia_release_v172128_hold', 'blocked-recovery', false );
			return $backup;
		}
		$errors = array();
		$run    = nadlan_utopia_read_run();
		...
		delete_option( 'nadlan_utopia_release_v172128' );
		delete_option( 'nadlan_utopia_release_v172128_manifest' );
		delete_option( 'nadlan_utopia_release_v172128_run' );
		if ( $operator_hold ) {
			update_option( 'nadlan_utopia_release_v172128_hold', 'operator', false );
		} else {
			delete_option( 'nadlan_utopia_release_v172128_hold' );
			delete_option( 'nadlan_utopia_backup_v172128' );
		}
		return true;
	}
}
...
if ( ! function_exists( 'nadlan_utopia_finish_release_transaction' ) ) {
	function nadlan_utopia_finish_release_transaction( $transaction, $commit, $post_ids = array() ) {
		...
		foreach ( array(
			'nadlan_utopia_release_v172128',
			'nadlan_utopia_release_v172128_manifest',
			'nadlan_utopia_release_v172128_run',
			'nadlan_utopia_release_v172128_error',
		) as $key ) {
			wp_cache_delete( $key, 'options' );
		}
		return $result === false
...
if ( ! function_exists( 'nadlan_utopia_seed_v172128' ) ) {
	function nadlan_utopia_seed_v172128() {
		if ( nadlan_utopia_release_complete() ) { return; }
		if ( get_option( 'nadlan_utopia_release_v172128_hold', false ) !== false ) { return; }
		...
		try {
			...
			foreach ( array(
				nadlan_utopia_transaction_set_option( 'nadlan_utopia_release_v172128_manifest', $release_manifest ),
				nadlan_utopia_transaction_set_option( 'nadlan_utopia_release_v172128', '1' ),
				nadlan_utopia_transaction_delete_option( 'nadlan_utopia_release_v172128_error' ),
				nadlan_utopia_transaction_delete_option( 'nadlan_utopia_release_v172128_run' ),
			) as $option_result ) {
			...
			update_option( 'nadlan_utopia_release_v172128_error', array( 'time' => gmdate( 'c' ), 'message' => $message ), false );
		} finally {
			nadlan_utopia_release_lock( $lock );
		}
	}
}
add_action( 'init', 'nadlan_utopia_seed_v172128', 40 );

if ( ! function_exists( 'nadlan_utopia_resume_release' ) ) {
	function nadlan_utopia_resume_release() {
		$hold = get_option( 'nadlan_utopia_release_v172128_hold', false );
		...
		if ( ! delete_option( 'nadlan_utopia_backup_v172128' ) &&
			get_option( 'nadlan_utopia_backup_v172128', false ) !== false ) {
...
if ( defined( 'WP_CLI' ) && WP_CLI && class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'nadlan utopia-restore-v172128', function () {
		...
		try {
			...
			WP_CLI::success( 'UTOPIA release 1.72.128 was restored and placed on operator hold.' );
		} finally {
			nadlan_utopia_release_lock( $lock );
		}
	} );
	WP_CLI::add_command( 'nadlan utopia-resume-v172128', function () {
		if ( ! nadlan_utopia_recovery_context_allowed() ) {
			WP_CLI::error( 'Run this command with --user=<administrator> for a user who has manage_options and unfiltered_html.' );
		}
		$lock = nadlan_utopia_acquire_release_lock();
...
if ( ! function_exists( 'nadlan_utopia_final_content_filter' ) ) {
	function nadlan_utopia_final_content_filter( $content ) {
		...
		if ( ! nadlan_utopia_is_family( $post_id ) || ! function_exists( 'nadlan_showroom_engine_shortcode' ) ) {
			return $content;
		}
