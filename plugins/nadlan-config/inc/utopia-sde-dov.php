...
if ( ! function_exists( 'nadlan_utopia_acquire_release_lock' ) ) {
	function nadlan_utopia_acquire_release_lock() {
		$key   = 'nadlan_utopia_release_v172139_lock';
...
if ( ! function_exists( 'nadlan_utopia_release_complete' ) ) {
	function nadlan_utopia_release_complete() {
		return (string) nadlan_utopia_db_option( 'nadlan_utopia_release_v172139', '' ) === '1';
	}
}
...
if ( ! function_exists( 'nadlan_utopia_read_run' ) ) {
	function nadlan_utopia_read_run() {
		$run = get_option( 'nadlan_utopia_release_v172139_run', false );
...
if ( ! function_exists( 'nadlan_utopia_store_run' ) ) {
	function nadlan_utopia_store_run( $run, $create = false ) {
		...
		$stored = $create
			? add_option( 'nadlan_utopia_release_v172139_run', $run, '', 'no' )
			: update_option( 'nadlan_utopia_release_v172139_run', $run, false );
		if ( ! $stored && get_option( 'nadlan_utopia_release_v172139_run', false ) !== $run ) {
...
if ( ! function_exists( 'nadlan_utopia_read_backup' ) ) {
	function nadlan_utopia_read_backup() {
		$backup = get_option( 'nadlan_utopia_backup_v172139', false );
...
if ( ! function_exists( 'nadlan_utopia_store_backup' ) ) {
	function nadlan_utopia_store_backup( $backup ) {
		...
		$changed = update_option( 'nadlan_utopia_backup_v172139', $backup, false );
		if ( ! $changed ) {
			$current = get_option( 'nadlan_utopia_backup_v172139', false );
			if ( $current !== $backup ) {
...
if ( ! function_exists( 'nadlan_utopia_backup_state' ) ) {
	function nadlan_utopia_backup_state( $posts ) {
		if ( get_option( 'nadlan_utopia_backup_v172139', false ) !== false ) {
		...
		$backup['checksum'] = nadlan_utopia_backup_checksum( $backup );
		if ( ! add_option( 'nadlan_utopia_backup_v172139', $backup, '', 'no' ) ) {
...
if ( ! function_exists( 'nadlan_utopia_restore_backup' ) ) {
	function nadlan_utopia_restore_backup( $extra_resources = array(), $operator_hold = false, $run_token = '' ) {
		...
		if ( is_wp_error( $backup ) ) {
			update_option( 'nadlan_utopia_release_v172139_hold', 'blocked-recovery', false );
			return $backup;
		}
		$errors = array();
		$run    = nadlan_utopia_read_run();
		...
		delete_option( 'nadlan_utopia_release_v172139' );
		delete_option( 'nadlan_utopia_release_v172139_manifest' );
		delete_option( 'nadlan_utopia_release_v172139_run' );
		if ( $operator_hold ) {
			update_option( 'nadlan_utopia_release_v172139_hold', 'operator', false );
		} else {
			delete_option( 'nadlan_utopia_release_v172139_hold' );
			delete_option( 'nadlan_utopia_backup_v172139' );
		}
		return true;
	}
}
...
if ( ! function_exists( 'nadlan_utopia_finish_release_transaction' ) ) {
	function nadlan_utopia_finish_release_transaction( $transaction, $commit, $post_ids = array() ) {
		...
		foreach ( array(
			'nadlan_utopia_release_v172139',
			'nadlan_utopia_release_v172139_manifest',
			'nadlan_utopia_release_v172139_run',
			'nadlan_utopia_release_v172139_error',
		) as $key ) {
			wp_cache_delete( $key, 'options' );
		}
		return $result === false
...
if ( ! function_exists( 'nadlan_utopia_seed_v172139' ) ) {
	function nadlan_utopia_seed_v172139() {
		if ( nadlan_utopia_release_complete() ) { return; }
		if ( get_option( 'nadlan_utopia_release_v172139_hold', false ) !== false ) { return; }
		...
		try {
			...
			foreach ( array(
				nadlan_utopia_transaction_set_option( 'nadlan_utopia_release_v172139_manifest', $release_manifest ),
				nadlan_utopia_transaction_set_option( 'nadlan_utopia_release_v172139', '1' ),
				nadlan_utopia_transaction_delete_option( 'nadlan_utopia_release_v172139_error' ),
				nadlan_utopia_transaction_delete_option( 'nadlan_utopia_release_v172139_run' ),
			) as $option_result ) {
			...
			update_option( 'nadlan_utopia_release_v172139_error', array( 'time' => gmdate( 'c' ), 'message' => $message ), false );
		} finally {
			nadlan_utopia_release_lock( $lock );
		}
	}
}
add_action( 'init', 'nadlan_utopia_seed_v172139', 40 );

if ( ! function_exists( 'nadlan_utopia_resume_release' ) ) {
	function nadlan_utopia_resume_release() {
		$hold = get_option( 'nadlan_utopia_release_v172139_hold', false );
		...
		if ( ! delete_option( 'nadlan_utopia_backup_v172139' ) &&
			get_option( 'nadlan_utopia_backup_v172139', false ) !== false ) {
...
if ( defined( 'WP_CLI' ) && WP_CLI && class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'nadlan utopia-restore-v172139', function () {
		...
		try {
			...
			WP_CLI::success( 'UTOPIA release 1.72.139 was restored and placed on operator hold.' );
		} finally {
			nadlan_utopia_release_lock( $lock );
		}
	} );
	WP_CLI::add_command( 'nadlan utopia-resume-v172139', function () {
		if ( ! nadlan_utopia_recovery_context_allowed() ) {
			WP_CLI::error( 'Run this command with --user=<administrator> for a user who has manage_options and unfiltered_html.' );
		}
		$lock = nadlan_utopia_acquire_release_lock();
