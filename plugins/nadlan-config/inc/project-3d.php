...
(function(){
	...
	function init(root){
		...
		var activeUnit=null;
		var activeFloor=floors[0] || maxFloor;
		...
		/* Start with the building overview. A stored unit id must not crop the first showroom view. */
		...
		function unitCameraOrbit(u){
			var orbit=u&&u.camera_orbit?String(u.camera_orbit):'';
			if(orbit&&window.innerWidth>760&&/\sauto\s*$/.test(orbit)){
				return orbit.replace(/\sauto\s*$/,' 38m');
			}
			return orbit;
		}
		function unitCameraTarget(value){
			var target=value?String(value).trim():'';
			var parts=target.split(/\s+/);
			if(parts.length===3&&parts.every(function(part){return /^-?\d+(\.\d+)?$/.test(part)})){
				return parts.map(function(part){return part+'m'}).join(' ');
			}
			return target;
		}
		...
		function refreshModelViewerRender(){
			if(!modelViewer){return}
			try{
				modelViewer.setAttribute('data-camera-refresh',String(Date.now()));
				void modelViewer.offsetHeight;
				modelViewer.removeAttribute('data-camera-refresh');
			}catch(e){}
		}
		function syncModelViewerCamera(){
			...
			window.requestAnimationFrame(function(){applyModelViewerCamera(orbit,target);refreshModelViewerRender()});
			window.setTimeout(function(){applyModelViewerCamera(orbit,target);refreshModelViewerRender()},280);
			window.setTimeout(refreshModelViewerRender,900);
		}
...
add_action(
	...
	function () {
		...
		wp_register_style( 'nadlan-p3d', '', array(), '1.69.22' );
		...
		wp_register_script( 'nadlan-p3d', '', array(), '1.69.22', true );
...
add_filter(
	...
	function ( $out ) {
		...
		$out['project_3d'] = array(
			...
			'model_viewer_selected_repaint_v16922' => true,
