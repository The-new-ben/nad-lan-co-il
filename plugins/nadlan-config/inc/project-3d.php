...
(function(){
	...
	function init(root){
		...
		function refreshModelViewerRender(orbit,target){
			if(!modelViewer){return}
			try{
				if(orbit){modelViewer.setAttribute('camera-orbit',orbit)}
				if(target){modelViewer.setAttribute('camera-target',target)}
			...
			modelViewer.addEventListener('click',function(e){
				...
				if(typeof modelViewer.positionAndNormalFromPoint==='function'){
					...
					selectUnitFromModelSurfacePoint(p,'model-surface-hit').then(function(hit){
						if(!hit){selectNearestModelPickFromPoint(p,'model-surface-near-pick')}
					});
					return;
				}
...
add_action(
	...
	function () {
		...
		wp_register_style( 'nadlan-p3d', plugins_url( '../assets/css/project-3d-premium.css', __FILE__ ), array(), '1.69.49' );
		...
		wp_register_script( 'nadlan-p3d', '', array(), '1.69.49', true );
