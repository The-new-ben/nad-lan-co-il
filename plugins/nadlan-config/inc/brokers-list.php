<?php
/**
 * /brokers/ becomes the brokers list (owner order 24.9.2026): "the 'brokers' menu item leads to a list of brokers,
 * and Meital appears with her photo and everything. Show she is premium without writing premium."
 *
 * Built only from the design system's components (inc/design-system.php, assets/nlds/nlds.css):
 *  - PageHeader: breadcrumbs, the one H1 "מתווכי נדל״ן", a lead with live numbers;
 *  - BrokerFeatureCard for a broker on a paid plan (nl_tier pro or studio) or pinned by the admin: the real portrait
...
if ( ! function_exists( 'nadlan_bl_brokers' ) ) {
	/** Published broker cards: array( featured => ids, rest => city => ids ). */
	function nadlan_bl_brokers() {
		$ids = get_posts( array( 'post_type' => 'nadlan_professional', 'post_status' => 'publish', 'numberposts' => 600, 'fields' => 'ids',
			'no_found_rows' => true, 'meta_query' => array( array( 'key' => 'profession', 'value' => 'metavech' ) ) ) );
		$feat = array(); $rest = array();
		foreach ( $ids as $id ) {
			if ( get_post_meta( $id, 'is_demo', true ) || 'demo_seed' === (string) get_post_meta( $id, 'source', true ) ) { continue; }
			$tier   = (string) get_post_meta( $id, 'nl_tier', true );
			$pinned = in_array( (string) get_post_meta( $id, 'is_pinned', true ), array( '1', 'yes', 'true', 'on' ), true );
			if ( $pinned || in_array( $tier, array( 'pro', 'studio' ), true ) ) { $feat[] = (int) $id; continue; }
			$city = trim( (string) get_post_meta( $id, 'city', true ) );
			$rest[ '' !== $city ? $city : 'ערים נוספות' ][] = (int) $id;
		}
		$order = array( 'תל אביב יפו', 'ירושלים', 'הרצליה', 'אילת', 'כפר שמריהו', 'סביון', 'קיסריה' );
		uksort( $rest, function ( $a, $b ) use ( $order ) {
			$ia = array_search( $a, $order, true ); $ib = array_search( $b, $order, true );
			$ia = false === $ia ? 99 : $ia; $ib = false === $ib ? 99 : $ib;
			return $ia === $ib ? strcmp( $a, $b ) : $ia - $ib;
		} );
		foreach ( $rest as $c => $list ) {
			usort( $list, function ( $a, $b ) { return strcmp( get_the_title( $a ), get_the_title( $b ) ); } );
			$rest[ $c ] = $list;
		}
		return array( 'featured' => $feat, 'rest' => $rest );
	}
}

if ( ! function_exists( 'nadlan_bl_active_listings' ) ) {
	function nadlan_bl_active_listings( $pro ) {
		$q = get_posts( array( 'post_type' => 'nadlan_property', 'post_status' => 'publish', 'numberposts' => 200, 'fields' => 'ids', 'no_found_rows' => true,
			'meta_query' => array( array( 'key' => 'nl_broker_id', 'value' => (string) $pro ), array( 'relation' => 'OR', array( 'key' => 'nl_status', 'value' => 'active' ), array( 'key' => 'nl_status', 'compare' => 'NOT EXISTS' ) ) ) ) );
		// one listing is one card, in whatever language it is published: count the Hebrew ones
		return count( array_filter( $q, function ( $id ) { return '' === (string) get_post_meta( $id, 'nl_lang', true ) || 'he' === (string) get_post_meta( $id, 'nl_lang', true ); } ) );
	}
}

if ( ! function_exists( 'nadlan_bl_feature_card' ) ) {
	function nadlan_bl_feature_card( $id ) {
		$name   = function_exists( 'nadlan_prof_person_name' ) ? (string) nadlan_prof_person_name( $id ) : get_the_title( $id );
		$role   = function_exists( 'nadlan_dir_prof_label' ) ? nadlan_dir_prof_label( 'metavech', $id ) : 'מתווך';
		$brand  = trim( (string) get_post_meta( $id, 'company_name', true ) );
		$city   = trim( (string) get_post_meta( $id, 'city', true ) );
		$lic    = trim( (string) get_post_meta( $id, 'license_number', true ) );
		$areas  = array_slice( array_filter( array_map( 'trim', explode( ',', (string) get_post_meta( $id, 'areas_served', true ) ) ) ), 0, 6 );
		$count  = nadlan_bl_active_listings( $id );
		...
		$site   = (int) get_post_meta( $id, 'nl_site_he', true );
		$href   = $site > 0 && 'publish' === get_post_status( $site ) ? get_permalink( $site ) : get_permalink( $id );
		$verified = function_exists( 'nadlan_dir_registry_verified' ) ? nadlan_dir_registry_verified( $id ) : false;
		$photo  = has_post_thumbnail( $id ) ? get_the_post_thumbnail( $id, 'medium_large', array( 'alt' => $name, 'loading' => 'eager', 'style' => 'width:100%;height:100%;object-fit:cover;object-position:50% 22%' ) ) : '<span class="nlds-mono" aria-hidden="true">' . esc_html( mb_substr( $name, 0, 1 ) ) . '</span>';
		ob_start(); ?>
<article class="nlds-bfeat">
	<figure class="nlds-bfeat__portrait"><?php echo $photo; // phpcs:ignore ?></figure>
	<div class="nlds-bfeat__body">
		<h3 class="nlds-bfeat__name"><a href="<?php echo esc_url( $href ); ?>"><?php echo esc_html( $name ); ?></a></h3>
	...
	<div class="nlds-bfeat__cta">
		<?php if ( '' !== $wa ) : ?><a class="nlds-btn nlds-btn--primary" target="_blank" rel="noopener" href="https://wa.me/<?php echo esc_attr( $wa ); ?>?text=<?php echo rawurlencode( 'שלום ' . $name . ', אשמח לתאם סיור באחד הנכסים שלך (nad-lan.co.il)' ); ?>"><?php echo nlds_icon( 'whatsapp' ); // phpcs:ignore ?><span>תיאום סיור</span></a><?php endif; ?>
		<a class="nlds-btn nlds-btn--secondary" href="<?php echo esc_url( $href ); ?>"><span><?php echo 'מתווכת' === $role ? 'לעמוד המתווכת' : 'לעמוד המתווך'; ?></span><?php echo nlds_icon( 'forward' ); // phpcs:ignore ?></a>
	</div>
</article>
		<?php
		return ob_get_clean();
...
if ( ! function_exists( 'nadlan_bl_html' ) ) {
...
<div class="nlds nlbl" dir="rtl" lang="he">
	...
	<section class="nlbl-city" id="<?php echo esc_attr( $slug( $c ) ); ?>" aria-label="<?php echo esc_attr( 'מתווכים ב' . $c ); ?>">
		...
		<div class="nlds-dgrid"><?php foreach ( $list as $id ) { echo nadlan_bl_card( $id ); } // phpcs:ignore ?></div>
	</section>
	<?php endforeach; ?>
	<?php endif; ?>
...
add_filter( 'the_content', function ( $content ) {
	if ( '0' === (string) get_option( 'nadlan_brokers_list', '1' ) || ! is_page( 7645 ) || ! in_the_loop() || ! is_main_query() ) { return $content; }
	// the offer below keeps its words; its H1 becomes an H2 under the list's one H1
	$content = preg_replace( '/<h1\b([^>]*)>(.*?)<\/h1>/is', '<h2$1>$2</h2>', $content, 1 );
	return nadlan_bl_html() . $content;
}, 20 );

/* "מתווכים" in the header, after "אנשי מקצוע", server-side in the header part's markup (crawlable, no JS) */
add_filter( 'render_block', function ( $html, $block ) {
	if ( '0' === (string) get_option( 'nadlan_brokers_list', '1' ) || false === strpos( (string) $html, 'id="nlpc-primary-nav"' ) || false !== strpos( (string) $html, 'href="/brokers/"' ) ) { return $html; }
