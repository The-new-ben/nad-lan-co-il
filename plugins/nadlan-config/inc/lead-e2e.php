...
if ( ! function_exists( 'nadlan_lead_e2e_clean_fields' ) ) {
	function nadlan_lead_e2e_clean_fields( $fields ) {
		...
		$out = array(
			...
			'building'     => sanitize_text_field( (string) ( $fields['building'] ?? '' ) ),
			'availability' => sanitize_text_field( (string) ( $fields['availability'] ?? '' ) ),
			'market_note'  => sanitize_textarea_field( (string) ( $fields['market_note'] ?? '' ) ),
			...
			'project_slug'      => sanitize_title( (string) ( $fields['project_slug'] ?? '' ) ),
			'project_title'     => sanitize_text_field( (string) ( $fields['project_title'] ?? '' ) ),
			'project_wp_id'     => absint( $fields['project_wp_id'] ?? 0 ),
			'direction'         => sanitize_key( (string) ( $fields['direction'] ?? '' ) ),
			'unit_status'       => sanitize_key( (string) ( $fields['unit_status'] ?? '' ) ),
			'consent'           => ! empty( $fields['consent'] ) ? 1 : '',
			'consent_text'      => sanitize_textarea_field( (string) ( $fields['consent_text'] ?? '' ) ),
			/* Capture the evidence timestamp at the trusted persistence boundary;
			 * never accept a browser/caller supplied consent time as authoritative. */
			'consent_recorded'  => ! empty( $fields['consent'] ) ? current_time( 'mysql', true ) : '',
...
if ( ! function_exists( 'nadlan_lead_e2e_notify_admin' ) ) {
	function nadlan_lead_e2e_notify_admin( $lead_id, $card_id, $fields, $reason = 'fallback' ) {
		...
		$lines = array(
			...
			'פרויקט: ' . ( $fields['project_title'] ?: $fields['project_slug'] ),
			'דירה/קו: ' . $fields['unit'],
			'קומה: ' . $fields['floor'],
			'חדרים: ' . $fields['rooms'],
			'שטח: ' . $fields['sqm'],
			'כיוון: ' . $fields['direction'],
			'סטטוס יחידה: ' . $fields['unit_status'],
			'הסכמה לפנייה: ' . ( ! empty( $fields['consent'] ) ? 'נקלטה' : 'לא נקלטה' ) .
				( ! empty( $fields['consent_recorded'] ) ? ' (' . $fields['consent_recorded'] . ' UTC)' : '' ),
			'',
			'הודעה:',
			$fields['message'],
			'',
