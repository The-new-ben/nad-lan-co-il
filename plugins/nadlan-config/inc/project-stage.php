...
if ( ! function_exists( 'nadlan_ps_parts' ) ) {
	...
	function nadlan_ps_parts( $ps, $h1, $map ) {
		...
		$src = is_readable( dirname( __DIR__ ) . '/assets/project-stage/' . $ps['dir'] . '/city.json' )
			? '<p class="nlps-src">מקורות הבמה: קו המגרש והבניינים הקיימים סביבו לפי עיריית <span>תל אביב-יפו</span> (שכבות התוכניות והמבנים, 9.2026); הפרויקטים ברובע לפי העמודים שלהם באתר.</p>'
			: '';
...
if ( ! function_exists( 'nadlan_ps_compose' ) ) {
	...
	function nadlan_ps_compose( $html, $ps ) {
		...
		$page = '<div class="nlps-page" dir="rtl" lang="he">' . $parts['hero'] . $lead . $parts['cta'] . $parts['stagebox'] . $parts['rail'] . $parts['facts'] . $parts['below'] . $parts['tour'] . $parts['deals'] . '</div>';
