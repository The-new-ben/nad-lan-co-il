...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/profile-emoji/deploy250.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "profile-emoji-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PE250/1.0"
...
NS = 'nadlan-pe250-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-pe250-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
EMOJI = ["\U0001F3A5", "\U0001F310", "\U0001F4CD", "\U0001F441", "\u2728"]
CHECKS = [
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', "תיאום סיור", 'class="nlds nlpp-video"', 'class="nlpp-pin"', "צפיות בפרופיל"], EMOJI),
...
# ---------------------------------------------------------------- main
FILES = ["inc/professional-profile.php"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("pe250" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("pe250b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-250.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-250.json"), "w", encoding="utf-8"), indent=2)
