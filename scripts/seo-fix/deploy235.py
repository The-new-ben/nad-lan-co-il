...
QA = os.path.join(REPO, "docs", "qa", "seo-fix-2026-09-24")
...
NS = 'nadlan-seo235-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-seo235-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
def rollback(created_any):
    print("[rollback] restoring .bak235 files (seo-gaps.php stays on disk, unused once nadlan-config.php is restored)")
    for rel in ("nadlan-config.php", "inc/bulk-project-seo.php", "inc/showroom-engine.php", "inc/premium-catalog.php", "inc/showroom-support.php", "assets/showroom-engine/i18n.js"):
        print("  ", rel, ops({"restore": rel}, "restore " + rel).get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/bulk-project-seo.php", "inc/showroom-engine.php", "inc/premium-catalog.php", "inc/showroom-support.php", "assets/showroom-engine/i18n.js", "inc/seo-gaps.php"]
NEWFILES = {"inc/seo-gaps.php"}
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-seo235-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'einstein-lab', 'smart-404' )"
    if text.count(anchor) != 1:
        raise SystemExit(f"FATAL module-list anchor x{text.count(anchor)}")
    text = text.replace(anchor, "'einstein-lab', 'smart-404', 'seo-gaps' )")
    ...
    bad = verify_pages("seo235" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("seo235b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-235.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-235.json"), "w", encoding="utf-8"), indent=2)
