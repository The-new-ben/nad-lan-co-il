...
QA = os.path.join(REPO, "docs", "qa", "legal-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-LG254/1.0"
...
NS = 'nadlan-lg254-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/legal-links.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-lg254-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">'], []),
]
LEGAL = {
    "privacy": {"title": "מדיניות פרטיות", "content": io.open(os.path.join(REPO, "docs", "legal", "privacy.html"), encoding="utf-8").read()},
    "terms": {"title": "תנאי שימוש", "content": io.open(os.path.join(REPO, "docs", "legal", "terms.html"), encoding="utf-8").read()},
}
LEGAL_PAGES = [
    ("/privacy/", ["מדיניות פרטיות</h1>", "חוק הגנת הפרטיות", "info@nad-lan.co.il", "Hotjar", 'class="nlpc-footer-legal"']),
    ("/terms/", ["תנאי שימוש</h1>", "אינו מתווך", "הדמיה", 'class="nlpc-footer-legal"'] ),
    ("/", ['class="nlpc-footer-legal"', 'href="https://nad-lan.co.il/terms/">תנאי שימוש</a>', 'href="https://nad-lan.co.il/privacy/">מדיניות פרטיות</a>', ">הצהרת נגישות</a>"]),
]
H1_EXACTLY_ONE = ["/brokers/", "/professionals/meital-katzir/"]
PL = None
...
# ---------------------------------------------------------------- main
FILES = ["inc/legal-links.php"]
NEWFILES = {"inc/legal-links.php"}
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-fg251-ops-", "x-tmp-fg252-ops-", "x-tmp-bl253-ops-", "x-tmp-lg254-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'brokers-list', 'firgun' ) as $nadlan_mod"
    if text.count(anchor) != 1:
        raise SystemExit("FATAL module anchor x%d" % text.count(anchor))
    text = text.replace(anchor, "'brokers-list', 'firgun', 'legal-links' ) as $nadlan_mod")
    ...
    bad = verify_pages("lg254" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("lg254b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-254.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-254.json"), "w", encoding="utf-8"), indent=2)
    print(f"RELEASE {new} LIVE")
    lg = ops({"legal": LEGAL}, "legal pages")["legal"]
    print("[legal]", lg)
    ops({"purge": 1}, "purge")
    time.sleep(3)
    for path, need in LEGAL_PAGES:
        s5, html5 = page(path + "?nlv=lg254p" + str(int(time.time())))
        body5 = body_of(html5)
        miss = [n for n in need if n not in html5]
        print(f"[legal] {'OK ' if s5 == 200 and not miss and (path == '/' or body5.count('<h1') == 1) else 'BAD'} {s5} {path} missing={miss} h1={body5.count('<h1')}")
