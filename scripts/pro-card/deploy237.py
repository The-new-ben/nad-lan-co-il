# -*- coding: utf-8 -*-
"""Release 1.72.237 (24.9.2026): one header and one breadcrumb on a professional card; the narrow article on phones.

Owner, 24.9.2026: "a professional card has two headings, not good; handle duplicate headings and breadcrumbs,
the site hierarchy matters" and "Meital's article is too narrow".
  inc/professional-profile.php  the hero is the one header: its name is the H1; a real portrait replaces the
                                initials when the card has a photo; register contractors show their register number
                                and branches (they lived only in the removed second header)
  inc/directory.php             the second header (its own H1 and band) is no longer printed; its quote script still loads
  inc/breadcrumbs.php           one visible trail per page: the plugin trail stays out where the theme prints Yoast's
                                (professional cards, listings without an owner); Yoast's trail gains the profession level
  inc/layout-fixes.php          NEW: phones, RTL: the full-width content box kept 72px on the left and 24px on the right
  nadlan-config.php             version bump + 'layout-fixes' in the module list, both on the LIVE text

Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/pro-card/deploy237.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "pro-card-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PRO237/1.0"
...
NS = 'nadlan-pro237-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/professional-profile.php', 'inc/directory.php', 'inc/breadcrumbs.php', 'inc/layout-fixes.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-pro237-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
MOYAL = "/professionals/%d7%9e%d7%95%d7%99%d7%90%d7%9c-%d7%99%d7%a0%d7%99%d7%91/"
CHECKS = [
    # path, must be in the raw HTML, must NOT be in the body
    ("/", [], []),
    ("/projects/rainbow-tel-aviv/", ["<h1"], []),
    ("/projects/h-infinity-somail-tel-aviv/", ["<h1"], []),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', "nlpp-photo", "מתווכים</a>"], ['<div class="nlpf"', '<nav class="nlbc"', '<h2 class="nlpp-name">']),
    (MOYAL, ['<h1 class="nlpp-name">', "רשום בפנקס הקבלנים, מספר", "תחומי רישום: שיפוצים", "קבלנים</a>"], ['<div class="nlpf"', '<nav class="nlbc"']),
    ("/professionals/demo-shira-golan-mortgage/", ['<h1 class="nlpp-name">'], ['<div class="nlpf"']),
    ("/properties/bialik-rg-4r-demo/", ['class="yoast-breadcrumbs"'], ['<nav class="nlbc"']),
    ("/properties/tzukei-aviv-3-rooms-berlin-for-sale/", ['<nav class="nlbc"', 'id="nadlan-layout-fixes"'], ['class="yoast-breadcrumbs"']),
]
...
def rollback(created_any):
    print("[rollback] restoring .bak237 files (layout-fixes.php stays on disk, unused once nadlan-config.php is restored)")
    for rel in ("nadlan-config.php", "inc/professional-profile.php", "inc/directory.php", "inc/breadcrumbs.php"):
        print("  ", rel, ops({"restore": rel}, "restore " + rel).get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/professional-profile.php", "inc/directory.php", "inc/breadcrumbs.php", "inc/layout-fixes.php"]
NEWFILES = {"inc/layout-fixes.php"}
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-pro237-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'smart-404', 'seo-gaps' )"
    if text.count(anchor) != 1:
        raise SystemExit(f"FATAL module-list anchor x{text.count(anchor)}")
    text = text.replace(anchor, "'smart-404', 'seo-gaps', 'layout-fixes' )")
    ...
    bad = verify_pages("pro237" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("pro237b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-237.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-237.json"), "w", encoding="utf-8"), indent=2)
