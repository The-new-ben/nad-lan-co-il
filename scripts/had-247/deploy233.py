# -*- coding: utf-8 -*-
"""HAD-247 polish release 1.72.233 (phone: the floor label wraps inside the frame; the camera stands a little further back on narrow screens). Based on deploy232.py.

HAD-247 polish release 1.72.232 (phone: building-section tabs in a 2x2 grid; one floor marker that carries the balcony). Based on deploy231.py.

HAD-247 follow-up release 1.72.231 (the portal block at the_content priority 11, after wpautop, so the facade and plan SVGs render and no <br /> lands between the tabs). Based on deploy229.py.
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-had247-ops-233-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
def verify_pages(tag):
    ...
    for path, need, never in CHECKS:
        ...
        # 1.72.231: wpautop must no longer reach into the portal block (</p> inside <svg>, <br /> between the tabs)
        if path in LISTING_PATHS and re.search(r"<svg[^>]*>\s*</p>", body):
            hit.append("wpautop </p> inside an svg")
        tog = body.find('<div class="nlps-toggle"')
        if path in LISTING_PATHS and tog > -1 and "<br />" in body[tog:body.find("</div>", tog)]:
            hit.append("wpautop <br /> between the tabs")
...
# ---------------------------------------------------------------- main
FILES = ["inc/property-showroom.php"]
...
try:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-233.json"), "w", encoding="utf-8"), indent=2)
