...
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/property-showroom.php', 'inc/property-gallery.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-had247-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    (OFAKIM, ["<figure class=\"nlps-cover\"", "nadlan-pgal-js", "2 כיווני אוויר"], ["<model-viewer", "nadlan-model-viewer-js", "<section class=\"nlps-facade\"", "מפה חיה"]),
...
def rollback(created_gallery):
    ...
    for rel in ("inc/property-showroom.php", "nadlan-config.php"):
        print("  ", rel, ops({"restore": rel}, "restore " + rel).get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
new_gallery = open(os.path.join(PLUG, "inc", "property-gallery.php"), "rb").read()
new_showroom = open(os.path.join(PLUG, "inc", "property-showroom.php"), "rb").read()
php_lint(new_gallery, "inc/property-gallery.php")
php_lint(new_showroom, "inc/property-showroom.php")
head_showroom = git_head("plugins/nadlan-config/inc/property-showroom.php")
...
try:
    ...
    cur_show = live_get("inc/property-showroom.php")
    cur_gal = live_get("inc/property-gallery.php")
    cur_main = live_get("nadlan-config.php")
    if cur_show.get("missing") or cur_main.get("missing"):
        raise SystemExit("FATAL live plugin files missing")
    live_show = base64.b64decode(cur_show["b64"])
    live_main = base64.b64decode(cur_main["b64"])
    print(f"[drift] live property-showroom.php {md5(live_show)[:10]} vs git HEAD {md5(head_showroom)[:10] if head_showroom else '-'}")
    if head_showroom is None or md5(live_show) != md5(head_showroom):
        raise SystemExit("FATAL: live property-showroom.php differs from git HEAD; diff it before replacing it")
    if not cur_gal.get("missing"):
        raise SystemExit("FATAL: inc/property-gallery.php already exists on the server")
    stamp = time.strftime("%Y%m%dT%H%M%S")
    open(os.path.join(QA, "live-backup", f"property-showroom.php.{stamp}.live"), "wb").write(live_show)
    ...
    print(f"[plan] {old} -> {new}; property-gallery.php new; property-showroom.php replaced")
    if DRY:
        print("[dry] no writes")
        raise SystemExit(0)

    try:
        put("inc/property-gallery.php", new_gallery, expect="missing")
        created = True
        put("inc/property-showroom.php", new_showroom, expect=md5(live_show))
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"), "files": {
        "inc/property-gallery.php": md5(new_gallery), "inc/property-showroom.php": md5(new_showroom), "nadlan-config.php": md5(new_main)},
        "live_before": {"inc/property-showroom.php": md5(live_show), "nadlan-config.php": md5(live_main)}},
        open(os.path.join(QA, "deploy-result.json"), "w", encoding="utf-8"), indent=2)
