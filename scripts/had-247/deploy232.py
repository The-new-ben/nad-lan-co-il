...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-had247-ops-232-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
try:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-232.json"), "w", encoding="utf-8"), indent=2)
