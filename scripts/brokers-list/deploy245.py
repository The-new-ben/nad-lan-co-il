...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/brokers-list/deploy243.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24", "r245")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-BL245/1.0"
...
NS = 'nadlan-bl245-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/design-system.php', 'inc/brokers-list.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-bl245-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/", ['href="/brokers/">מתווכים</a>', "nadlan-nlds-css"], []),
    ("/projects/rainbow-tel-aviv/", ["<h1", 'href="/brokers/">מתווכים</a>'], []),
    ("/brokers/", ['<h1 class="nlds-pagehead__title">מתווכי נדל״ן</h1>', 'class="nlds-bfeat"', 'class="nlds-procard"', "מיטל קציר", 'id="join"', "icons/profession-broker.svg", "icons/card-pin.svg"], ['nlds-procard__av" aria-hidden="true"><svg']),
    ("/about/", ['href="/brokers/">מתווכים</a>'], []),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">'], []),
    ("/wp-content/plugins/nadlan-config/assets/nlds/nlds.css", ["--nlds-space-20:20px", "site layer", ":root body .nlds .nlds-procard__name{", "@media (max-width: 600px){"], ["}.nlds-procard__name {", "\n.nlds-procard__name {"]),
    ("/wp-content/plugins/nadlan-config/assets/nlds/icons/profession-broker.svg", ["<svg", "M34 14l4-4"], []),
    ("/wp-content/plugins/nadlan-config/assets/nlds/icons/card-pin.svg", ["<svg"], []),
]
H1_EXACTLY_ONE = ["/brokers/", "/about/", "/projects/rainbow-tel-aviv/"]
...
# ---------------------------------------------------------------- main
ICONS = sorted("assets/nlds/icons/" + f for f in os.listdir(os.path.join(PLUG, "assets", "nlds", "icons")) if f.endswith(".svg"))
FILES = ["assets/nlds/nlds.css", "inc/brokers-list.php", "inc/design-system.php"] + ICONS
NEWFILES = set(ICONS)
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        # 1.72.243 and 244 went live today and are committed together with this release: the live file must be what the last of them wrote
        prev = json.load(open(os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24", "r244", "deploy-result-244.json"), encoding="utf-8"))["files"].get(rel)             or json.load(open(os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24", "deploy-result-243.json"), encoding="utf-8"))["files"].get(rel)
        if md5(LIVE[rel]) != prev:
            raise SystemExit(f"FATAL: live {rel} is not what release 243/244 wrote; diff it before replacing it")
    ...
    bad = verify_pages("bl245" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("bl245b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-245.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-245.json"), "w", encoding="utf-8"), indent=2)
