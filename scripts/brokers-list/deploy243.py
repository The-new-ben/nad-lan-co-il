...
QA = os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-BL243/1.0"
...
NS = 'nadlan-bl243-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-bl243-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/brokers/", ['<h1 class="nlds-pagehead__title">מתווכי נדל״ן</h1>', 'class="nlds-bfeat"', 'class="nlds-procard"', "מיטל קציר", 'id="join"'], []),
    ("/about/", ['href="/brokers/">מתווכים</a>'], []),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">'], []),
    ("/wp-content/plugins/nadlan-config/assets/nlds/nlds.css", ["nlds-bfeat"], []),
]
...
def rollback(created_any):
    print("[rollback] restoring nadlan-config.php (the new files stay on disk, unused)")
    print("  ", ops({"restore": "nadlan-config.php"}, "restore nadlan-config.php").get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["assets/nlds/nlds.css", "inc/design-system.php", "inc/brokers-list.php"]
NEWFILES = {"assets/nlds/nlds.css", "inc/design-system.php", "inc/brokers-list.php"}
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl243-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'pro-network', 'page-h1' )"
    if text.count(anchor) != 1:
        raise SystemExit(f"FATAL module-list anchor x{text.count(anchor)}")
    text = text.replace(anchor, "'pro-network', 'page-h1', 'design-system', 'brokers-list' )")
    ...
    bad = verify_pages("bl243" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("bl243b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-243.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-243.json"), "w", encoding="utf-8"), indent=2)
