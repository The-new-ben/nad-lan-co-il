...
QA = os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24", "r244")
...
NS = 'nadlan-bl244-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-bl244-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/wp-content/plugins/nadlan-config/assets/nlds/nlds.css", ["--nlds-space-20:20px", "site layer"], []),
]
...
def rollback(created_any):
    print("[rollback] restoring .bak244 files")
    for rel in ("nadlan-config.php", "assets/nlds/nlds.css", "inc/brokers-list.php"):
        print("  ", rel, ops({"restore": rel}, "restore " + rel).get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["assets/nlds/nlds.css", "inc/brokers-list.php"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = json.load(open(os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24", "deploy-result-243.json"), encoding="utf-8"))["files"].get(rel)
        if md5(LIVE[rel]) != prev:
            raise SystemExit(f"FATAL: live {rel} is not what release 243 wrote; diff it before replacing it")
    ...
    bad = verify_pages("bl244" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("bl244b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-244.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-244.json"), "w", encoding="utf-8"), indent=2)
