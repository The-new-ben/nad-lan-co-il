...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/brokers-list/deploy253.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24", "r253")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-BL253/1.0"
...
NS = 'nadlan-bl253-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/brokers-list.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-bl253-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/brokers/", ['<h1 class="nlds-pagehead__title">מתווכי נדל״ן</h1>', 'class="nlds-bfeat"', 'class="nlbl-more"', "עוד 44 מתווכים בתל אביב יפו", "עוד 24 מתווכים בירושלים"], []),
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-fg251-ops-", "x-tmp-fg252-ops-", "x-tmp-bl253-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    bad = verify_pages("bl253" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("bl253b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-253.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-253.json"), "w", encoding="utf-8"), indent=2)
    print(f"RELEASE {new} LIVE")
    s4, html4 = page("/brokers/?nlv=bl253c" + str(int(time.time())))
    body4 = body_of(html4)
    print("[brokers] cards in the page:", body4.count('class="nlds-procard"'), "| hidden behind more:", body4.count('class="nlbl-more"'), "sections")
