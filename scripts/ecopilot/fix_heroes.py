...
CODE = (
    ...
    "\tregister_rest_route( 'nadlan-hero-fix/v1', '/go', array(\n"
    "\t\t'methods' => 'POST',\n"
    ...
    "\t\t\t\t$out[ $pid ]['old_3d_image'] = (string) get_post_meta( $pid, 'project_3d_image', true );\n"
    "\t\t\t\t$out[ $pid ]['old_poster'] = (string) get_post_meta( $pid, 'project_model_poster', true );\n"
    "\t\t\t\tupdate_post_meta( $pid, 'project_3d_image', esc_url_raw( $url ) );\n"
    "\t\t\t\t$out[ $pid ]['new'] = $url;\n"
    "\t\t\t}\n"
...
s, r = req("POST", "/wp-json/nadlan-hero-fix/v1/go", {"token": TOKEN})
