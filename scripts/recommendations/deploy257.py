# -*- coding: utf-8 -*-
"""Release 1.72.257 (24.9.2026): "המלצות", not "פרגונים" (owner's word), and how many recommendations a professional
received, with a thumbs-up ("an indication of the number of recommendations... something that makes a challenge").
Designed first in Claude Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 11: RecCount on EndorsementStrip, ProfileHeader,
ProfessionalCard, BrokerSquare).
  inc/firgun.php                 every visible text says המלצה/ממליצים; the strip's heading carries the count
  inc/pro-network.php            nadlan_endorse_counts() (one query for a list, published endorsers only) and
                                 nadlan_endorse_count_html(); the error texts renamed
  inc/design-system.php          nlds_icon('thumb')
  inc/professional-profile.php   the count under the name (from one up), linking to the strip
  inc/brokers-list.php           the count in each card's foot; within a city, more recommendations first (the challenge)
  inc/project-stage.php          the count in the rail's BrokerSquare
  assets/nlds/nlds.css           rebuilt (RecCount)
  nadlan-config.php              version bump only
After the code: page 8101 (/firgun/, the slug stays) is retitled "המלצות בין אנשי מקצוע".
...
QA = os.path.join(REPO, "docs", "qa", "recommendations-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-RC257/1.0"
...
NS = 'nadlan-rc257-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
	...
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['retitle'] ) ) {
				$pid = 8101;
				$pg  = get_post( $pid );
				if ( ! $pg || 'page' !== $pg->post_type ) { return new WP_Error( 'retitle', 'not the page', array( 'status' => 400 ) ); }
				$res = array( 'before' => $pg->post_title, 'slug' => $pg->post_name );
				if ( false !== mb_strpos( $pg->post_title, 'פרגונ' ) ) {
					$u = wp_update_post( array( 'ID' => $pid, 'post_title' => 'המלצות בין אנשי מקצוע' ), true );
					$res['updated'] = is_wp_error( $u ) ? $u->get_error_message() : 1;
				}
				$yt = (string) get_post_meta( $pid, '_yoast_wpseo_title', true );
				if ( '' !== $yt && false !== mb_strpos( $yt, 'פרגונ' ) ) { update_post_meta( $pid, '_yoast_wpseo_title', str_replace( 'פרגונים', 'המלצות', $yt ) ); $res['yoast_title'] = 1; }
				$yd = (string) get_post_meta( $pid, '_yoast_wpseo_metadesc', true );
				if ( '' !== $yd && false !== mb_strpos( $yd, 'פרג' ) ) { $res['yoast_desc_needs_hand'] = $yd; }
				clean_post_cache( $pid );
				$res['after'] = get_the_title( $pid );
				$out['retitle'] = $res;
			}
			if ( ! empty( $b['head'] ) ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/firgun.php', 'inc/pro-network.php', 'inc/design-system.php', 'inc/professional-profile.php', 'inc/brokers-list.php', 'inc/project-stage.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-rc257-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    (RB, ['id="nlps"', 'class="nlbsq"', 'class="nlbslot"', 'id="nlps-view"', '<h1 id="nl-project-page-title" class="nlps-h1">', 'id="nlpjx-map"', 'class="nl-lead"', 'nlcp-projctx', 'nadlan-project-article', 'רישיון תיווך', 'פרסומת', 'רוצה להופיע כאן?', 'לקבלת תוכניות ומחירים', 'assets/project-stage/bridge.js?ver=1.72.257', 'nadlan-ps-importmap'],
     ['על המפה החיה למטה', 'id="nl-project-page-title" class="screen-reader-text"']),
    ...
    ("/brokers/", ['<h1 class="nlds-pagehead__title">'], ['id="nlps"', 'פרגונ']),
    ("/firgun/", ['המלצה היא מילה טובה של איש מקצוע'], ['פרגון הוא המלצה', 'איך מפרגנים']),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">'], ['id="nlps"', 'פרגונ']),
]
...
def rollback(created_any):
    print("[rollback] restoring .bak257 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["inc/firgun.php", "inc/pro-network.php", "inc/design-system.php", "inc/professional-profile.php", "inc/brokers-list.php", "inc/project-stage.php", "assets/nlds/nlds.css"]
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-fg251-ops-", "x-tmp-fg252-ops-", "x-tmp-bl253-ops-", "x-tmp-lg254-ops-", "x-tmp-ps255-ops-", "x-tmp-ps256-ops-", "x-tmp-rc257-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "legal-2026-09-24/deploy-result-254.json", "brokers-list-2026-09-24/r253/deploy-result-253.json", "firgun-2026-09-24/r252/deploy-result-252.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("rc257" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("rc257o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("rc257b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("rc257p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-257.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-257.json"), "w", encoding="utf-8"), indent=2)
    print(f"RELEASE {new} LIVE")
    print("[retitle]", ops({"retitle": 1}, "retitle")["retitle"])
    ops({"purge": 1}, "purge")
    time.sleep(3)
    s7, h7 = page("/firgun/?nlv=rc257t" + str(int(time.time())))
    b7 = body_of(h7)
    print("[firgun page]", s7, "h1 says המלצות:", "המלצות בין אנשי מקצוע</h1>" in b7, "| old word left:", b7.count("פרגונ"))
