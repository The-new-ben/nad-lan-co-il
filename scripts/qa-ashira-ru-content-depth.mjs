...
const PATTERN = 'patterns/project-showroom-ashira-v2-ru.php';
const DRAFT = 'docs/wp-drafts/ashira-sde-dov-ru-v2-draft.json';
const OUT = 'docs/qa/ashira-ru-content-depth-report.json';
...
function words(text) {
	return String(text || '').match(/[A-Za-zА-Яа-яЁё0-9][A-Za-zА-Яа-яЁё0-9"'’.-]*/g) || [];
}
...
const failures = [];
const banned = /(SEO|CMS|CRM|lead|leads|engine|template|prototype|project manager|supplier|contractor|internal|strategy|factory|fallback|placeholder|mock|monetization|Codex|Claude|лид|воронк|монетизац|поставщик|подрядчик|шаблон|прототип|внутренн|заглушк|мок|фабрик|стратеги)/i;
...
if (!pattern.includes('Источники и примечания о надежности')) failures.push('pattern missing Russian sources section');
...
if (/[\u0590-\u05ff]/.test(visible)) failures.push('Russian page visible text contains Hebrew characters');
if (!/[А-Яа-яЁё]/.test(visible)) failures.push('Russian page visible text contains no Cyrillic text');
