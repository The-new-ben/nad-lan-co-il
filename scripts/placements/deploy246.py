...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/placements/deploy246.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "placements-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PL246/1.0"
...
NS = 'nadlan-pl246-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/placements.php', 'inc/brokers-list.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-pl246-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/", ['href="/brokers/">מתווכים</a>', "nadlan-nlds-css"], ['class="nlds nlpl"']),
    ("/projects/rainbow-tel-aviv/", ["<h1"], ['class="nlds nlpl"']),
    ("/brokers/", ['<h1 class="nlds-pagehead__title">מתווכי נדל״ן</h1>', '<h3 class="nlds-bfeat__name">', "תיאום סיור", 'class="nlds-procard"', "מיטל קציר", 'id="join"'], ['class="nlds nlpl"']),
    ("/property-value/", ["<h1"], []),
...
# ---------------------------------------------------------------- main
FILES = ["inc/brokers-list.php", "inc/placements.php"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = json.load(open(os.path.join(REPO, "docs", "qa", "brokers-list-2026-09-24", "r245", "deploy-result-245.json"), encoding="utf-8"))["files"].get(rel)
        if prev:
            if md5(LIVE[rel]) != prev:
                raise SystemExit(f"FATAL: live {rel} is not what release 245 wrote; diff it before replacing it")
    ...
    bad = verify_pages("pl246" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("pl246b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-246.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-246.json"), "w", encoding="utf-8"), indent=2)
    print(f"RELEASE {new} LIVE")

    # the placement, only on healthy code; a failure switches the placement off and keeps the code
    seeded = ops({"seed_pl": PL}, "seed placement")["seed_pl"]
    print("[pl] placement", seeded["id"], "created" if seeded["created"] else "updated", seeded["status"], "active", seeded["meta"].get("pl_active"))
    ops({"purge": 1}, "purge")
    time.sleep(3)
    bad_pl = verify_placement("pl246p" + str(int(time.time())))
    if bad_pl:
        ops({"purge": 1}, "purge again")
        time.sleep(12)
        bad_pl = verify_placement("pl246q" + str(int(time.time())))
    if bad_pl:
        print("[pl] FAIL on", bad_pl, "- switching the placement off:", ops({"pl_off": seeded["id"]}, "pl off").get("pl_off"))
        ops({"purge": 1}, "purge")
    json.dump({"placement": seeded, "failed_pages": bad_pl}, open(os.path.join(QA, "placement-246.json"), "w", encoding="utf-8"), indent=2, ensure_ascii=False)
    print("PLACEMENT", "LIVE" if not bad_pl else "OFF")
finally:
    bridge_down()
