# -*- coding: utf-8 -*-
"""Release 1.72.239 (24.9.2026): the professionals network data layer (owner FOCUS).

  inc/pro-network.php   NEW: professions for every part of a home (5 groups, 34 professions, plural and female forms);
                        endorsements ("פרגונים", positive by construction, one per pair, 12 a month) and invitations
                        with a join link; REST /pros/search, /endorse, /endorse/<id>; a cron backfill of `trade` for
                        the 2,698 register contractors. Nothing prints on the site yet (the screens come from the design system).
  inc/directory.php     labels and gendered forms fall back to the registry for the new professions
  inc/breadcrumbs.php   the profession level falls back to the registry plural
  nadlan-config.php     version bump + 'pro-network' in the module list, both on the LIVE text

Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/pro-network/deploy239.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "pro-network-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-NET239/1.0"
...
NS = 'nadlan-net239-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/pro-network.php', 'inc/directory.php', 'inc/breadcrumbs.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-net239-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', "מתווכים</a>", 'id="nadlan-insights"'], []),
    ("/professionals/%d7%9e%d7%95%d7%99%d7%90%d7%9c-%d7%99%d7%a0%d7%99%d7%91/", ['<h1 class="nlpp-name">', "קבלנים</a>"], []),
    ("/professionals/", [], []),
    ("/wp-json/nadlan/v1/pros/search?q=%D7%9E%D7%99%D7%98%D7%9C", ['"id":7833'], []),
]
...
def rollback(created_any):
    print("[rollback] restoring nadlan-config.php, directory.php, breadcrumbs.php (pro-network.php stays on disk, unused)")
    for rel in ("nadlan-config.php", "inc/directory.php", "inc/breadcrumbs.php"):
        print("  ", rel, ops({"restore": rel}, "restore " + rel).get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/pro-network.php", "inc/directory.php", "inc/breadcrumbs.php"]
NEWFILES = {"inc/pro-network.php"}
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-net239-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'insights', 'placements' )"
    if text.count(anchor) != 1:
        raise SystemExit(f"FATAL module-list anchor x{text.count(anchor)}")
    text = text.replace(anchor, "'insights', 'placements', 'pro-network' )")
    ...
    bad = verify_pages("net239" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("net239b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-239.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-239.json"), "w", encoding="utf-8"), indent=2)
