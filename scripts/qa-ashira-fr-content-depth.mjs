...
const PATTERN = 'patterns/project-showroom-ashira-v2-fr.php';
const DRAFT = 'docs/wp-drafts/ashira-sde-dov-fr-v2-draft.json';
const OUT = 'docs/qa/ashira-fr-content-depth-report.json';
...
function words(text) {
	return String(text || '').match(/[A-Za-zÀ-ÖØ-öø-ÿ0-9][A-Za-zÀ-ÖØ-öø-ÿ0-9"'’.-]*/g) || [];
}
...
const failures = [];
const banned = /(SEO|CMS|CRM|lead|leads|engine|template|prototype|project manager|supplier|contractor|internal|strategy|factory|fallback|placeholder|mock|monetization|Codex|Claude|referencement|moteur de recherche|gestionnaire de projet|fournisseur|entonnoir)/i;
...
if (!pattern.includes('Sources et notes de fiabilite')) failures.push('pattern missing French sources section');
...
if (/[\u0590-\u05ff]/.test(visible)) failures.push('French page visible text contains Hebrew characters');
