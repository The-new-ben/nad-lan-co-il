# -*- coding: utf-8 -*-
"""Release 1.72.238 (24.9.2026): private broker insights and the broker placement mechanism.

  inc/insights.php     NEW: a beacon on a professional's card, a broker's site pages and owned listings counts views,
                       the broker's WhatsApp, calls, tour requests and the SITE's WhatsApp; a private dashboard
                       (NadLan Ops > "מתווכים: תנועה ועסקאות") and a private deals ledger; REST /nadlan/v1/ev (public
                       beacon, no IP stored) and /nadlan/v1/ev-stats (admin only)
  inc/placements.php   NEW: "שיבוץ מתווכים": who, on which pages, when, after which section; labelled as the broker's
                       ad with the licence number; nothing shows until a placement is published and active
  nadlan-config.php    version bump + 'insights', 'placements' in the module list, both on the LIVE text

Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/insights/deploy238.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "insights-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-INS238/1.0"
...
NS = 'nadlan-ins238-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/insights.php', 'inc/placements.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ins238-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/", [], ['id="nadlan-insights"']),
    ("/projects/rainbow-tel-aviv/", ["<h1"], ['id="nadlan-insights"']),
    ("/projects/h-infinity-somail-tel-aviv/", ["<h1"], []),
    ("/professionals/meital-katzir/", ['id="nadlan-insights"', '<h1 class="nlpp-name">'], ['class="nlpl"']),
    ("/properties/tzukei-aviv-3-rooms-berlin-for-sale/", ['id="nadlan-insights"'], []),
    ("/brokers/meital-katzir/", ['id="nadlan-insights"'], []),
    ("/property-value/", [], ['class="nlpl"', 'id="nadlan-insights"']),
    ("/about/", [], ['id="nadlan-insights"']),
]
...
def rollback(created_any):
    print("[rollback] restoring nadlan-config.php (insights.php and placements.php stay on disk, unused)")
    print("  ", "nadlan-config.php", ops({"restore": "nadlan-config.php"}, "restore nadlan-config.php").get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/insights.php", "inc/placements.php"]
NEWFILES = {"inc/insights.php", "inc/placements.php"}
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-ins238-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'seo-gaps', 'layout-fixes' )"
    if text.count(anchor) != 1:
        raise SystemExit(f"FATAL module-list anchor x{text.count(anchor)}")
    text = text.replace(anchor, "'seo-gaps', 'layout-fixes', 'insights', 'placements' )")
    ...
    bad = verify_pages("ins238" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("ins238b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-238.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-238.json"), "w", encoding="utf-8"), indent=2)
