# -*- coding: utf-8 -*-
"""Release 1.72.272 (25.9.2026): the stage's first picture in under 4 s on a mid Android (the site loop, R6 part 1). Designed
first in Claude Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 29: ProjectStage's first picture). Measured with
scripts/project-stage/firstpic.py (4x CPU, Slow 4G): the picture showed at 9.1 s, made by the stage's script after every other
script had run; the page's layout came from the footer, so until the whole page had arrived the stage sat low and small.
  inc/project-stage.php                     the first picture in the page's HTML (a phone-sized file and the full one in srcset,
                                            high priority, the stage's crop); the head's preload names the same set; the stage's
                                            script is preloaded quietly; the page's layout CSS moves from the footer to the end
                                            of the head (its rules win by selector and !important, not by order)
  assets/project-stage/rainbow/poster-716.jpg NEW: the poster for phones (716 x 660, 38 KB; the full one is 146 KB)
  assets/project-stage/rainbow/stage.js     adopts the page's picture as its poster (nothing fetched or painted twice); the
                                            quarter's pins never cover the stage's own hint, light switch or caption (seen in
                                            the first view: "פירסט" and "מגדל איינשטיין" sat on "בחרו קומה במגדל")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS272/1.0"
...
NS = 'nadlan-ps272-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps272-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
def rollback(created_any):
    print("[rollback] restoring .bak272 files")
...
# ---------------------------------------------------------------- main
NEWFILES = {"assets/project-stage/rainbow/poster-716.jpg"}
# the phone poster first: the page names it in its srcset
FILES = ["assets/project-stage/rainbow/poster-716.jpg", "assets/project-stage/rainbow/stage.js", "inc/project-stage.php"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-271.json", "project-stage-2026-09-24/deploy-result-270.json", "project-stage-2026-09-24/deploy-result-269.json", "project-stage-2026-09-24/deploy-result-268.json", "project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps272" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps272o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps272b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps272p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-272.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-272.json"), "w", encoding="utf-8"), indent=2)
