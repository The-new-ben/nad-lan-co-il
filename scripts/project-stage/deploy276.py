"""Release 1.72.276 (25.9.2026): the homepage's first picture on a phone sooner (the site loop H1.2b; Linear HAD-297).
Live 1.72.275 on a mid Android (4x CPU, Slow 4G): the photo at 4.1-4.5 s, started at 0.76 s and shared the line with the
pictures of the bands below it.

  assets/home/hero-m-780.jpg  the phone crop at 720 x 960, 74 KB (was 780 x 1040, 92 KB); srcset and preload say 720w
  inc/home-v3.php             the project cards' and the listings' pictures fetchpriority="low" (they are below the
                              first screen and lazy), so the photo is fetched first
  nadlan-config.php           version bump, on the LIVE text

Same safety chain as deploy275.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/project-stage/deploy276.py [--dry | --rollback]
"""
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS276/1.0"
...
NS = 'nadlan-ps276-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps276-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
def rollback(created_any):
    print("[rollback] restoring .bak276 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["assets/home/hero-m-780.jpg", "inc/home-v3.php"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-275.json", "project-stage-2026-09-24/deploy-result-274.json", "project-stage-2026-09-24/deploy-result-273.json", "project-stage-2026-09-24/deploy-result-272.json", "project-stage-2026-09-24/deploy-result-271.json", "project-stage-2026-09-24/deploy-result-270.json", "project-stage-2026-09-24/deploy-result-269.json", "project-stage-2026-09-24/deploy-result-268.json", "project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps276" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps276o" + str(int(time.time()))):
        bad = ["order"]
    if healthy and not bad and not verify_home_order("ps276h" + str(int(time.time()))):
        bad = ["home-order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps276b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps276p" + str(int(time.time()))):
            bad = ["order"]
        if healthy and not bad and not verify_home_order("ps276q" + str(int(time.time()))):
            bad = ["home-order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-276.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-276.json"), "w", encoding="utf-8"), indent=2)
