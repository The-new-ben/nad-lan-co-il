...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS255/1.0"
...
NS = 'nadlan-ps255-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/project-stage.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps255-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    (RB, ['id="nlps"', 'class="nlbsq"', 'class="nlbslot"', 'id="nlps-view"', '<h1 id="nl-project-page-title" class="nlps-h1">', 'id="nlpjx-map"', 'class="nl-lead"', 'nlcp-projctx', 'nadlan-project-article', 'רישיון תיווך', 'פרסומת', 'רוצה להופיע כאן?', 'לקבלת תוכניות ומחירים', 'assets/project-stage/bridge.js', 'nadlan-ps-importmap'],
...
def rollback(created_any):
    print("[rollback] restoring .bak255 files, removing the new ones")
...
# ---------------------------------------------------------------- main
NEWFILES = {"inc/project-stage.php", "assets/project-stage/bridge.js", "assets/project-stage/rainbow/stage.js", "assets/project-stage/rainbow/stage.css", "assets/project-stage/rainbow/poster.jpg"}
FILES = ["assets/nlds/nlds.css"] + sorted(NEWFILES)
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-fg251-ops-", "x-tmp-fg252-ops-", "x-tmp-bl253-ops-", "x-tmp-lg254-ops-", "x-tmp-ps255-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'firgun', 'legal-links' ) as $nadlan_mod"
    if text.count(anchor) != 1:
        raise SystemExit("FATAL module anchor x%d" % text.count(anchor))
    text = text.replace(anchor, "'firgun', 'legal-links', 'project-stage' ) as $nadlan_mod")
    ...
    bad = verify_pages("ps255" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps255o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps255b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps255p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-255.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-255.json"), "w", encoding="utf-8"), indent=2)
