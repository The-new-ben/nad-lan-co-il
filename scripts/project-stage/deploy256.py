...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS256/1.0"
...
NS = 'nadlan-ps256-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps256-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    (RB, ['id="nlps"', 'class="nlbsq"', 'class="nlbslot"', 'id="nlps-view"', '<h1 id="nl-project-page-title" class="nlps-h1">', 'id="nlpjx-map"', 'class="nl-lead"', 'nlcp-projctx', 'nadlan-project-article', 'רישיון תיווך', 'פרסומת', 'רוצה להופיע כאן?', 'לקבלת תוכניות ומחירים', 'assets/project-stage/bridge.js?ver=1.72.256', 'poster.jpg?ver=1.72.256', 'nadlan-ps-importmap'],
...
def rollback(created_any):
    print("[rollback] restoring .bak256 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["assets/project-stage/bridge.js", "assets/project-stage/rainbow/stage.js", "assets/project-stage/rainbow/poster.jpg", "inc/project-stage.php"]
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-fg251-ops-", "x-tmp-fg252-ops-", "x-tmp-bl253-ops-", "x-tmp-lg254-ops-", "x-tmp-ps255-ops-", "x-tmp-ps256-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    bad = verify_pages("ps256" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps256o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps256b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps256p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-256.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-256.json"), "w", encoding="utf-8"), indent=2)
