"""Release 1.72.274 (25.9.2026): the homepage's first picture sooner (the site loop H1.1b; design system version 31;
Linear HAD-297). Live 1.72.273 on a mid Android (4x CPU, Slow 4G): the H1 at 3.1-3.3 s, the photo at 5.1-5.4 s (it was
17.0-17.7 s). The photo started at 0.7 s but shared the line with 1.9 MB, most of it never used on the home:

  inc/home-v3.php   on the Hebrew front page only:
                    - no model-viewer (285 KB) and none of the theme's old home showroom files: the home has no 3D viewer;
                    - no Stripe library (262 KB): the home has no payment form; a link with the membership plugin's own
                      parameters (?pms_...) keeps everything;
                    - both are dropped at print time (script_loader_tag), not by dequeue: other scripts list them as
                      dependencies, so the first attempt printed them anyway and its own check rolled it back;
                    - the renewal and rentals bands render when they come near (content-visibility), so their two
                      background pictures (350 KB and 414 KB) wait for the scroll.
  nadlan-config.php version bump, on the LIVE text

Same safety chain as deploy273.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/project-stage/deploy274.py [--dry | --rollback]
"""
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS274/1.0"
...
NS = 'nadlan-ps274-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps274-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
def rollback(created_any):
    print("[rollback] restoring .bak274 files")
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-273.json", "project-stage-2026-09-24/deploy-result-272.json", "project-stage-2026-09-24/deploy-result-271.json", "project-stage-2026-09-24/deploy-result-270.json", "project-stage-2026-09-24/deploy-result-269.json", "project-stage-2026-09-24/deploy-result-268.json", "project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps274" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps274o" + str(int(time.time()))):
        bad = ["order"]
    if healthy and not bad and not verify_home_order("ps274h" + str(int(time.time()))):
        bad = ["home-order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps274b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps274p" + str(int(time.time()))):
            bad = ["order"]
        if healthy and not bad and not verify_home_order("ps274q" + str(int(time.time()))):
            bad = ["home-order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-274.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-274.json"), "w", encoding="utf-8"), indent=2)
