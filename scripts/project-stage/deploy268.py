...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS268/1.0"
...
NS = 'nadlan-ps268-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps268-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
ORDER = ['<h1 id="nl-project-page-title" class="nlps-h1">', 'class="nl-lead"', 'nlps-hero__cta', 'id="nlps"', 'class="nlbsq"', 'class="nlpf"', 'id="nlps-view"', 'id="nlpjx-map"', 'class="nlpd"', 'nlcp-projctx', 'class="nl-projnotice"', 'nadlan-project-article']
...
def rollback(created_any):
    print("[rollback] restoring .bak268 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["inc/project-stage.php", "assets/nlds/nlds.css", "assets/project-stage/bridge.js", "assets/project-stage/rainbow/stage.js",
         "assets/project-stage/rainbow/stage.css", "assets/project-stage/rainbow/quarter.json", "assets/project-stage/rainbow/poster.jpg"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps268" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps268o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps268b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps268p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-268.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-268.json"), "w", encoding="utf-8"), indent=2)
