...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS261/1.0"
...
NS = 'nadlan-ps261-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/project-stage.php', 'inc/project-experience.php', 'inc/accessibility.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps261-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    (RB, ['class="nlps-page"', 'id="nlps"', 'id="nlps-t"', 'class="nlbsq"', 'class="nlbslot"', 'id="nlps-view"', 'id="nlps-view-k"', '<h1 id="nl-project-page-title" class="nlps-h1">', 'id="nlpjx-map"', 'class="nl-lead"', 'nlcp-projctx', 'nadlan-project-article', 'רישיון תיווך', 'פרסומת', 'רוצה להופיע כאן?', 'לקבלת תוכניות ומחירים', 'assets/project-stage/bridge.js?ver=1.72.261', 'poster.jpg?ver=1.72.261', '32.10354', 'nlps-ctawrap', 'nlps-hero__cta', 'class="nlpf"', 'class="nlprog"', 'nadlan-ps-faq', 'nadlan-ps-importmap', '<div class="nlps-kicker">', 'class="nlpd"', 'data-nlps-floor="6"', 'data-nlps-floor="13"', 'דירות שנמכרו בריינבו תל אביב, לפי קומה', 'בחרו קומה במגדל, ואחר כך דירה לדוגמה בטבעת הקומה.', '#nla11y{position:fixed;bottom:calc(20px'],
     ['על המפה החיה למטה', 'id="nl-project-page-title" class="screen-reader-text"', 'דירות 82-210', 'class="nlms"', 'class="nlcard-facts"', 'class="nlps-shell"', '<p class="nlps-kicker">', 'rainbow-tel-aviv-hero.jpg', 'בחירת דירה בתלת ממד', '#nla11y{position:fixed;top:50%']),
    ...
    ("/projects/duo-tel-aviv/", ['nadlan-project-article', 'class="nlcard-facts"'], ['id="nlps"', 'nadlan-ps-importmap']),
...
def rollback(created_any):
    print("[rollback] restoring .bak261 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["inc/project-stage.php", "assets/nlds/nlds.css", "inc/accessibility.php", "assets/project-stage/bridge.js", "assets/project-stage/rainbow/stage.js", "assets/project-stage/rainbow/stage.css"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps261" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps261o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps261b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps261p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-261.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-261.json"), "w", encoding="utf-8"), indent=2)
