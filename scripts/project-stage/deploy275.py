"""Release 1.72.275 (25.9.2026): the homepage's bands (the site loop H1.2; design system version 31, HomePage sections
4-13; Linear HAD-297). The Hebrew home is now rendered whole by inc/home-v3.php, in the design's order:

  inc/home-v3.php    the new top (H1.1) and then: new projects (a lead card, the page's own answer sentence, facts only
                     from the project's fields, "סיור וירטואלי" where a stage exists, links by city); the listings band
                     of home-v2 with its two placeholder tiles replaced by one "מפרסמים דירה?" line, the photos at
                     WordPress's 768 size, and its sale/rent tabs working (the rent grid was always shown); prices by
                     city as horizontal bars (the CBS figures of the old chart) with the four tools; real estate by
                     city (catalogue counts, the CBS average where there is one, links to projects, apartments and the
                     city prices page); the map; professionals; the magazine with its lead as a deep card without the
                     cropped picture; the three product bands as three service cards (their words, steps, buttons,
                     the video); the English band; the portal paragraph; the big footer. If the bands cannot be made,
                     the H1.1 page stays (fails open); option nadlan_home_v3_bands=off keeps H1.1.
  inc/i18n.php       Hebrew strings off the blacklist: "נכסים חדשים במערכת" -> "מודעות חדשות באתר", "המגזין" ->
                     "מגזין נדל״ן", no "מודל תלת-ממדי" in the renewal and rentals lines, "מסמנים את הדירות על הבניין",
                     "לחדר הפרויקט של הבניין" (was "+ הדגמה חיה")
  inc/drone-map.php  the map's Hebrew title "מפת הפרויקטים החדשים" (was "מפת הפרויקטים החיה"), its toggle line without
                     "חיה" and "תלת ממד", and the eyebrow in each page language instead of the English "LIVE"
  nadlan-config.php  version bump, on the LIVE text

Same safety chain as deploy274.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/project-stage/deploy275.py [--dry | --rollback]
"""
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS275/1.0"
...
NS = 'nadlan-ps275-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps275-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/projects/", ['class="nlpc-site-header"'], ['id="nlhp-top"', 'nadlan-hp-css', 'מפת רחפן חיה', ' LIVE</span>']),
    ("/premium/", [], ['מפת הפרויקטים החיה', ' LIVE</span>', 'id="nlhp-top"']),
...
def rollback(created_any):
    print("[rollback] restoring .bak275 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["inc/i18n.php", "inc/drone-map.php", "inc/home-v3.php"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-274.json", "project-stage-2026-09-24/deploy-result-273.json", "project-stage-2026-09-24/deploy-result-272.json", "project-stage-2026-09-24/deploy-result-271.json", "project-stage-2026-09-24/deploy-result-270.json", "project-stage-2026-09-24/deploy-result-269.json", "project-stage-2026-09-24/deploy-result-268.json", "project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps275" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps275o" + str(int(time.time()))):
        bad = ["order"]
    if healthy and not bad and not verify_home_order("ps275h" + str(int(time.time()))):
        bad = ["home-order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps275b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps275p" + str(int(time.time()))):
            bad = ["order"]
        if healthy and not bad and not verify_home_order("ps275q" + str(int(time.time()))):
            bad = ["home-order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-275.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-275.json"), "w", encoding="utf-8"), indent=2)
