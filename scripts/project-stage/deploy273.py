"""Release 1.72.273 (25.9.2026): the homepage's new top (the site loop H1.1; design system version 31, HomePage and
HomePagePhone; Linear HAD-297). The owner's order of 25.9 morning: the portal look kept (menus, mega menus, lots of
content and listings), above every competitor, with search words and speed in mind.

  inc/home-v3.php                  NEW. On the Hebrew front page with the skin on:
                                   - the header: one row, the categories as menus (details panels that work without a
                                     script: new projects, apartments for sale, for rent, prices, virtual tours, the
                                     magazine, professionals, real estate abroad), the language menu, "פרסום מודעה";
                                     on a phone a menu button opens the same panels as a sheet. It replaces the theme's
                                     header part on this page only;
                                   - the hero: the site's own coastline photo across the width (a portrait crop on
                                     phones), the H1 "נדל״ן: פרויקטים חדשים, דירות למכירה ומחירי דירות" over the sea, the
                                     live count, the search (the same tabs and script as before), popular searches;
                                   - the categories row (eight real pages);
                                   - an honest description with the live count, after the theme's: the old one promised
                                     "זמינות דירות לפי קומה ונוף", which is not true for most projects;
                                   - the first picture: a preload per media (phone 92 KB, desktop 121/187 KB) and the
                                     top's CSS at the end of the head. Local, on a mid Android (4x CPU, Slow 4G): LCP
                                     2.5-5.6 s; it was 17.0-17.7 s live.
                                   The bands below the hero are the skin's, unchanged. Language homes are untouched.
                                   It swaps the renderer's top by markers and fails open; ?hp=off shows the page without.
  assets/home/hero-*.jpg           NEW. The three crops of the site's photo (uploads/2026/07/nadlan-hero-israel-coastline-v2).
  nadlan-config.php                version bump + 'home-v3' in the module list, on the LIVE text

Same safety chain as deploy272.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/project-stage/deploy273.py [--dry | --rollback]
"""
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS273/1.0"
...
NS = 'nadlan-ps273-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/project-stage.php', 'inc/project-experience.php', 'inc/sdedov-teaser.php', 'inc/home-v3.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps273-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/en/", ['class="nlhv2-langbar"', 'class="nlcta-wa"', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));left:20px'], ['#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));right:20px', 'id="nlhp-top"', 'nadlan-hp-css']),
    ("/projects/", ['class="nlpc-site-header"'], ['id="nlhp-top"', 'nadlan-hp-css']),
    ...
    ("/", ['class="nlcta-wa"', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));right:20px', 'id="nlhp-top"', 'id="nlhp-hero"', 'class="nlhp-cats"', 'id="nlhp-nav"', 'class="nlhp-dd"', 'hero-m-780.jpg?ver=1.72.273 780w', 'hero-d-1400.jpg?ver=1.72.273 1400w', 'hero-d-1920.jpg?ver=1.72.273 1920w', 'media="(max-width: 760px)" fetchpriority="high"', 'media="(min-width: 761px)" fetchpriority="high"', '<style id="nadlan-hp-css">', 'id="nadlan-hp-js"', '<h1>נדל״ן: פרויקטים חדשים, דירות למכירה ומחירי דירות</h1>', 'content="נדל״ן בישראל: ', 'nlhv2-search', 'nlhv2-tabs', 'data-action="https://nad-lan.co.il/properties/" data-extra="listing_type=rent"', 'nlsa-band', 'nlhv2-mfoot', '/post-listing/', '/tour/sde-dov/'], ['id="nlps"', 'nadlan-ps-importmap', '#nla11y{position:fixed;top:50%', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));left:20px', 'class="nlhv2-langbar"', '<section class="nlsa-hero"', 'class="nlpc-site-header"', 'nadlan-hero-israel-coastline-v2.jpg', 'סיורים תלת־ממד'], ['זמינות דירות לפי קומה ונוף', 'nadlan-hero-israel-coastline', 'בחירת דירה מתוך הבניין', 'בחירת דירה בתלת ממד']),
...
HOME_ORDER = ['id="nlhp-top"', 'id="nlhp-hero"', '<h1>', 'class="nlhp-cats"', 'nlsa-band']
...
def verify_home_order(tag):
    ...
    ok = s == 200 and all(p > -1 for p in pos) and pos == sorted(pos) and body.count('id="nlhp-top"') == 1
...
def rollback(created_any):
    print("[rollback] restoring .bak273 files")
...
# ---------------------------------------------------------------- main
NEWFILES = {"assets/home/hero-m-780.jpg", "assets/home/hero-d-1400.jpg", "assets/home/hero-d-1920.jpg", "inc/home-v3.php"}
# the pictures first: the page names them
FILES = ["assets/home/hero-m-780.jpg", "assets/home/hero-d-1400.jpg", "assets/home/hero-d-1920.jpg", "inc/home-v3.php"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-272.json", "project-stage-2026-09-24/deploy-result-271.json", "project-stage-2026-09-24/deploy-result-270.json", "project-stage-2026-09-24/deploy-result-269.json", "project-stage-2026-09-24/deploy-result-268.json", "project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    a = "'legal-links', 'project-stage' ) as $nadlan_mod"
    if text.count(a) != 1:
        raise SystemExit(f"FATAL module list anchor x{text.count(a)}")
    text = text.replace(a, "'legal-links', 'project-stage', 'home-v3' ) as $nadlan_mod")
    ...
    bad = verify_pages("ps273" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps273o" + str(int(time.time()))):
        bad = ["order"]
    if healthy and not bad and not verify_home_order("ps273h" + str(int(time.time()))):
        bad = ["home-order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps273b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps273p" + str(int(time.time()))):
            bad = ["order"]
        if healthy and not bad and not verify_home_order("ps273q" + str(int(time.time()))):
            bad = ["home-order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-273.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-273.json"), "w", encoding="utf-8"), indent=2)
