...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS259/1.0"
...
NS = 'nadlan-ps259-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps259-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
ORDER = ['<h1 id="nl-project-page-title" class="nlps-h1">', 'nlps-hero__cta', 'class="nl-lead"', 'class="nlpf"', 'class="nl-projnotice"', 'id="nlps"', 'class="nlbsq"', 'id="nlps-view"', 'id="nlpjx-map"', 'nlcp-projctx', 'nadlan-project-article']
...
def rollback(created_any):
    print("[rollback] restoring .bak259 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["inc/project-stage.php", "assets/nlds/nlds.css"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps259" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps259o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps259b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps259p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-259.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-259.json"), "w", encoding="utf-8"), indent=2)
