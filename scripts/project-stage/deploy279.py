"""Release 1.72.279 (25.9.2026): the phone fix for 1.72.278 (Rainbow, the floor answers; Linear HAD-221).
Found in the live phone journey: the tap that opens the floor card also pressed the button that appeared under the finger
(the browser's click follows the touch) and opened the designer. The card's buttons now wait half a second after it opens.

  assets/project-stage/rainbow/stage.js   the guard
  nadlan-config.php                       version bump, on the LIVE text

Same safety chain as deploy278.py.   python scripts/project-stage/deploy279.py [--dry | --rollback]
"""
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS279/1.0"
...
NS = 'nadlan-ps279-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps279-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
def rollback(created_any):
    print("[rollback] restoring .bak279 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["assets/project-stage/rainbow/stage.js"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-278.json", "project-stage-2026-09-24/deploy-result-277.json", "project-stage-2026-09-24/deploy-result-276.json", "project-stage-2026-09-24/deploy-result-275.json", "project-stage-2026-09-24/deploy-result-274.json", "project-stage-2026-09-24/deploy-result-273.json", "project-stage-2026-09-24/deploy-result-272.json", "project-stage-2026-09-24/deploy-result-271.json", "project-stage-2026-09-24/deploy-result-270.json", "project-stage-2026-09-24/deploy-result-269.json", "project-stage-2026-09-24/deploy-result-268.json", "project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps279" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps279o" + str(int(time.time()))):
        bad = ["order"]
    if healthy and not bad and not verify_home_order("ps279h" + str(int(time.time()))):
        bad = ["home-order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps279b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps279p" + str(int(time.time()))):
            bad = ["order"]
        if healthy and not bad and not verify_home_order("ps279q" + str(int(time.time()))):
            bad = ["home-order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-279.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-279.json"), "w", encoding="utf-8"), indent=2)
