# -*- coding: utf-8 -*-
"""Release 1.72.271 (25.9.2026): the example apartment on the balcony too (the site loop, R5 part 3). Designed first in Claude
Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 versions 26-28: ApartmentTour's places and the balcony note).
  assets/project-stage/rainbow/tour/balcony-25{n,e,s,w}.jpg (+ -2k)  NEW: standing on the balcony in each direction
  assets/project-stage/rainbow/tour/living-25{n,e,s,w}.jpg (+ -2k, the card) rendered again: the tower's balconies are now
                                         drawn as the stage draws them (three waves around the tower, 0.7 to 3.6 m, a solid
                                         white parapet 1.05 m, the tower's own glass around), so every view matches the stage
  assets/project-stage/tour.js, tour.css two switches: the direction, and where one stands (in the living room, on the
                                         balcony); each keeps the other
  inc/project-stage.php                  the card's scenes: each direction in both places; every balcony view's caption
                                         says its size is an illustration and gives the record's limit (the design plan of
                                         5.2023: the tower's balconies project up to 2 m; the stage's waves reach 3.6 m)
  assets/project-stage/tour.css           the viewer's bottom is one column (the switches, then the caption): a long caption
                                         covered the direction bar on a phone
  nadlan-config.php                      version bump only

Release 1.72.270 (25.9.2026): the example apartment in the four directions of floor 25 (the site loop, R5 part 2). Designed
first in Claude Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 25: ApartmentTour's directions).
  assets/project-stage/rainbow/tour/living-25{n,e,s}.jpg (+ -2k)  NEW: north, east, south, rendered the same way as the west
  assets/project-stage/rainbow/tour/living-25w.jpg (+ -2k, -card) rendered again: a new Blender camera saw only 1 km, so the
                                         sky's dark lower half showed as the horizon (inland it read as a false sea); now the
                                         camera sees 60 km, the ground reaches 40 km and far land hazes as over Tel Aviv
  assets/project-stage/tour.js, tour.css the direction bar (one pressed, each direction facing its window), a caption per view
                                         (planned projects named where they stand in it), Tab moves through the controls
  assets/project-stage/bridge.js         the four directions to the viewer; it opens on the side picked on the stage
  inc/project-stage.php                  the card: floor 25 in the four directions, the page's own words for each
  nadlan-config.php                      version bump only

Release 1.72.269 (25.9.2026): the example apartment from the inside (the site loop, R5 part 1). Designed first in Claude
Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 24: ApartmentTour). No floor plans of Rainbow are public, so it is an
example apartment, labelled so on the card and in the viewer, with the caption "הדמיית פנים להמחשה בלבד".
  assets/project-stage/rainbow/tour/living-25w.jpg       NEW: our own Cycles 360° (scripts/interior/rainbow_interior.py):
                                                         a living room with an open kitchen as delivered, floor 25 (the eye
                                                         at 98.8 m), the tower's curved glass facing true west; through it
                                                         the city's buildings, the coast and the sea from our data
  assets/project-stage/rainbow/tour/living-25w-2k.jpg    NEW: the same at 2048 (it shows first)
  assets/project-stage/rainbow/tour/living-25w-card.jpg  NEW: a straight view through the window for the card
  assets/project-stage/tour.js, tour.css                 NEW: the full-screen viewer (a sphere in three.js), loaded on the button
  inc/project-stage.php                                  the card after the view and the map, before the sold apartments
  assets/project-stage/bridge.js                         the button opens the viewer (tour_open counted)
  assets/nlds/nlds.css                                   rebuilt from the design system version 24 (the card)
  nadlan-config.php                                      version bump only

Release 1.72.268 (25.9.2026): the quarter's legend under Rainbow's stage (the site loop, R4c). Designed first in Claude
Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 23: QuarterPins' legend; StageCity's source line in sa-ink2). The honest
form of the "time machine": seven of the nine projects publish no completion year, so the stage groups by each page's
status and shows a year only where a page gives one.
  inc/project-stage.php                    the legend under the stage: קיים היום (the city's buildings, 808) / בבנייה (the
                                           building itself and Einstein) / בשיווק (5) / בשלב ההיתר (2), counted from the data,
                                           with its source line
  assets/project-stage/rainbow/quarter.json each project's group from its page's status, and Einstein's 2030 from its page
  assets/project-stage/rainbow/stage.js    the masses in their group's colour; a dot on each project pin; focusPhase(): a group
                                           alone, the camera looking across the line from the building to the group until all
                                           fit (the limit lifted while shown); the building's own label in that overview; the
                                           card says "אכלוס צפוי 2030, לפי עמוד הפרויקט" where the page gives it
  assets/project-stage/rainbow/stage.css   the group dots on the pins, the building's label
  assets/project-stage/bridge.js           the chips wired to the stage (one pressed at a time; quarter_legend counted)
  assets/project-stage/rainbow/poster.jpg  the first picture with the group colours
  assets/nlds/nlds.css                     rebuilt from the design system version 23
  nadlan-config.php                        version bump only

Release 1.72.267 (25.9.2026): the real city around Rainbow (the site loop, R4b part B). Designed first in Claude Design
(artifact L9Nqz7Viv7K3MYeZrBc9s8 versions 21-22: StageCity).
  assets/project-stage/rainbow/city.json  NEW: 808 buildings standing today around the lot (Tel Aviv-Yafo GIS layer 513
                                          "מבנים": outline, recorded height, floors, year) and the Sde Dov plan's 21 lots with
                                          approved design plans plus the planned linear park (build_city_rainbow.py)
  assets/project-stage/rainbow/stage.js   loads city.json with its 3D library and draws the real buildings and the plan's lots
                                          in place of the generic volumes and plates, which came from no source (kept only as
                                          the fallback when the file is missing)
  assets/project-stage/rainbow/poster.jpg the first picture rendered again with the real city
  inc/project-stage.php                   the stage's source line under it, always visible, in the page's HTML
  assets/nlds/nlds.css                    rebuilt from the design system version 22 (the source line)
  nadlan-config.php                       version bump only

Release 1.72.266 (25.9.2026): Rainbow's own lot from the record (the site loop, R4b part A). Designed first in Claude
Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 20: StageLot).
  assets/project-stage/rainbow/stage.js   the lot follows its official line (TLV GIS plan layer, lot 111: long north to south,
                                          about 8,500 m²; the stage drew it 104 x 80 m turned the wrong way); the tower at
                                          its true point in the north-east corner (it stood ~30 m off, so every distance to the
                                          quarter was wrong); six boutique buildings around the courtyard by the design plan's
                                          rules (8 m apart, 12 m from the tower, 5 m back in the west; their exact places are
                                          an illustration); the two roof pools on the southern two; street trees along the line;
                                          no quarter pin may sit on the tower on screen, in front of it, behind it or on its roof
  assets/project-stage/rainbow/poster.jpg the first picture rendered again from the new lot, with the quarter's masses
  nadlan-config.php                       version bump only

Release 1.72.265 (25.9.2026): two defects seen on the live pages after 1.72.264 (the site loop, R4). Designed first in
Claude Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 19: QuarterPins, AccessibilityButton).
  assets/project-stage/rainbow/stage.js  two quarter pins never cover each other: the nearer one keeps its place, a farther
                                         one waits for another angle (live on a phone "פירסט" sat on "גינדי ווג"; a sampled
                                         orbit found overlaps in 8 of 24 steps on 1.72.264 and none on this code)
  inc/accessibility.php                  the button's corner follows the page's own language, like the WhatsApp pill: on the
                                         English, French and Russian pages it stood bottom-right on top of the pill's end
                                         (the site's locale is Hebrew on every page); now bottom-left there
  nadlan-config.php                      version bump only

Release 1.72.264 (25.9.2026): the quarter inside Rainbow's stage (the site loop, R4 part 1). Designed first in Claude
Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 18: QuarterPins).
  assets/project-stage/rainbow/quarter.json  NEW: our eight project pages around Rainbow (developer, status, floors and units
                                             from each page, the parcel point) and five places a buyer asks about (the
                                             light rail station, the school, the park, the beach, the Yarkon estuary; the
                                             find-place data), each with its distance and bearing from the tower and a source
                                             (scripts/project-stage/build_quarter_rainbow.py)
  assets/project-stage/rainbow/stage.js      the projects stand at their true places as pale translucent volumes with soft
                                             edges (schematic, by their floors; Rainbow stays the one solid building); a pin
                                             over each project and place, hidden behind Rainbow's tower and faded by
                                             distance; a pin opens a card (developer, status, floors, units, distance and
                                             direction, the source) with "מה רואים מריינבו לכיוונו", which turns the floor's
                                             direction to that exact bearing and names it ("לכיוון זוהי")
  assets/project-stage/rainbow/stage.css     the pins and the card (a bottom sheet on a phone), shielded from the theme
  assets/project-stage/bridge.js             the quarter to the stage; a direction toward a pin is named after the pin in the
                                             view's title and in the WhatsApp text; the pins are counted (quarter_pin)
  inc/project-stage.php                      the page reads the project's quarter.json into the stage's settings (a missing or
                                             broken file leaves the stage as it was)
  assets/nlds/nlds.css                       rebuilt from the design system version 18
  nadlan-config.php                          version bump only

Release 1.72.263 (25.9.2026): Hebrew place names read correctly (the site loop, X1; Linear HAD-287). 1.72.262 put the
maps' place names in Hebrew; without Mapbox's right-to-left text plugin their letters came out reversed ("גנידר תנחת").
  assets/project-stage/bridge.js    the view from the floor registers the plugin (once per page, lazily); its waiting text
                                    leaves the map's container when the map starts (Mapbox asks for an empty container)
  inc/project-experience.php        the area map registers the same plugin before it starts
  nadlan-config.php                 version bump only

Release 1.72.262 (25.9.2026): the small fixes on Rainbow's page (the site loop, X1; Linear HAD-287). Designed first in
Claude Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 17).
  assets/project-stage/bridge.js            the view from the floor names places in Hebrew (the local names)
  assets/project-stage/rainbow/poster.jpg   rendered at the stage's own aspect (716x660 x2), so the first picture matches the scene
  inc/project-experience.php                the area map: place names in the page's language; the layer chips and the markers'
                                            popups without emoji (the legend key stays)
  inc/project-stage.php                     one map per page (the surroundings band's static map is not shown); the planning
                                            step shows its date alone (it crowded the next step on a phone)
  inc/sdedov-teaser.php                     the Sde Dov tour box: the skin makes it a light card, so its text takes the skin's ink
  nadlan-config.php                         version bump only

Release 1.72.261 (25.9.2026): the owner's two rulings of the same night (the site loop, R3). Designed first in Claude
Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 version 15: ProjectStage, ProjectDeals, AccessibilityButton).
  assets/project-stage/rainbow/stage.js   example apartments: four per tower floor, one per side; a pick on the ring snaps
                                          to the apartment, its arc is drawn, the card says "דירה לדוגמה · קומה N" with a
                                          "לדוגמה" chip; the floor card's line comes from the page (floorNote); no floor is
                                          called a penthouse floor without a source; selectUnit("16-w")
  assets/project-stage/rainbow/stage.css  the "לדוגמה" chip
  assets/project-stage/bridge.js          the example apartments and the floor lines to the stage; the view says "לדוגמה";
                                          ?unit= keeps and opens an apartment; the deals table's buttons open a floor; the QA
                                          switch's regex (a heredoc had turned its \b into a backspace)
  inc/project-stage.php                   the settings (four sides, the sold apartments with sources, the developer's words
                                          per height) and the new ProjectDeals section after the view and the map
  inc/accessibility.php                   the accessibility button moves to the bottom corner at the start side (owner:
                                          "להזיז אותו לפינה התחתונה"); its panel opens upwards
  assets/nlds/nlds.css                    rebuilt from the design system version 15
  nadlan-config.php                       version bump only

Release 1.72.260 (25.9.2026): the building in the first fold (the site loop, R2; the recipe row 4 and rule A11). Designed
first in Claude Design (artifact L9Nqz7Viv7K3MYeZrBc9s8 versions 13-14: ProjectStage's page grid). The non-affiliation
notice goes back to opening the article section (the owner's order of 29.8.2026; 1.72.259 had lifted it under the facts
and the public-source audit caught it).
  inc/project-stage.php   the page top is one grid: on a wide screen the text column (title, lead, the two buttons),
                          the stage and the rail share the first fold; on a phone the stage comes right after the title and
                          the buttons; facts, progress, then the view next to the map. The lead stays the first paragraph
                          after the h1 in the source (the kicker is a div); the stage keeps an h2 for screen readers
  inc/milestones.php      one progress band per page: not printed where the project's page top is on
  inc/cards-render.php    one facts table per page: the project card's table is not printed there either
  assets/nlds/nlds.css    rebuilt from the design system version 14
  nadlan-config.php       version bump only
Before the code, the same night: the page's search and social fields (rainbow_meta.py) and the article's description of
the old engine (rainbow_content4.py) were made true.

Release 1.72.259 (24.9.2026): Rainbow's page top to the recipe (the site loop, R2). Designed first in Claude Design
(artifact L9Nqz7Viv7K3MYeZrBc9s8 version 12: ProjectFacts, ProjectProgress, the calls to action).
  inc/project-stage.php   two calls to action under the title (WhatsApp for plans and prices; "לבחירת קומה" to the stage);
                          six sourced quick facts and where the project stands, under the answer paragraph; the one-line
                          non-affiliation notice right under them; one FAQPage schema from the page's own visible questions
  assets/nlds/nlds.css    rebuilt (ProjectFacts, ProjectProgress, the hero's calls to action, the answer paragraph)
  nadlan-config.php       version bump only
After the code: scripts/project-stage/rainbow_content2.py --apply writes the answer paragraph from sourced facts, keeps
one FAQ (the buyer's ten questions) and corrects three answers.
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS271/1.0"
...
NS = 'nadlan-ps271-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/project-stage.php', 'inc/project-experience.php', 'inc/sdedov-teaser.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps271-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/projects/rainbow-tel-aviv-en/", ['#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));left:20px'], ['id="nlps"', 'nadlan-ps-importmap', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));right:20px']),
    ("/en/", ['class="nlcta-wa"', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));left:20px'], ['#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));right:20px']),
    ("/ru/", ['class="nlcta-wa"', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));left:20px'], ['#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));right:20px']),
    ...
    ("/", ['class="nlcta-wa"', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));right:20px'], ['id="nlps"', 'nadlan-ps-importmap', '#nla11y{position:fixed;top:50%', '#nla11y{position:fixed;bottom:calc(20px + env(safe-area-inset-bottom,0px));left:20px']),
...
def rollback(created_any):
    print("[rollback] restoring .bak271 files")
...
# ---------------------------------------------------------------- main
NEWFILES = {"assets/project-stage/rainbow/tour/balcony-25n.jpg", "assets/project-stage/rainbow/tour/balcony-25n-2k.jpg", "assets/project-stage/rainbow/tour/balcony-25e.jpg", "assets/project-stage/rainbow/tour/balcony-25e-2k.jpg", "assets/project-stage/rainbow/tour/balcony-25s.jpg", "assets/project-stage/rainbow/tour/balcony-25s-2k.jpg", "assets/project-stage/rainbow/tour/balcony-25w.jpg", "assets/project-stage/rainbow/tour/balcony-25w-2k.jpg"}
# the pictures first: the card lists only the scenes whose pictures are on the server
FILES = ["assets/project-stage/rainbow/tour/balcony-25n.jpg", "assets/project-stage/rainbow/tour/balcony-25n-2k.jpg", "assets/project-stage/rainbow/tour/balcony-25e.jpg", "assets/project-stage/rainbow/tour/balcony-25e-2k.jpg", "assets/project-stage/rainbow/tour/balcony-25s.jpg", "assets/project-stage/rainbow/tour/balcony-25s-2k.jpg", "assets/project-stage/rainbow/tour/balcony-25w.jpg", "assets/project-stage/rainbow/tour/balcony-25w-2k.jpg",
         "assets/project-stage/rainbow/tour/living-25n.jpg", "assets/project-stage/rainbow/tour/living-25n-2k.jpg", "assets/project-stage/rainbow/tour/living-25e.jpg", "assets/project-stage/rainbow/tour/living-25e-2k.jpg", "assets/project-stage/rainbow/tour/living-25s.jpg", "assets/project-stage/rainbow/tour/living-25s-2k.jpg", "assets/project-stage/rainbow/tour/living-25w.jpg", "assets/project-stage/rainbow/tour/living-25w-2k.jpg", "assets/project-stage/rainbow/tour/living-25w-card.jpg",
         "assets/project-stage/tour.js", "assets/project-stage/tour.css", "inc/project-stage.php"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-270.json", "project-stage-2026-09-24/deploy-result-269.json", "project-stage-2026-09-24/deploy-result-268.json", "project-stage-2026-09-24/deploy-result-267.json", "project-stage-2026-09-24/deploy-result-266.json", "project-stage-2026-09-24/deploy-result-265.json", "project-stage-2026-09-24/deploy-result-264.json", "project-stage-2026-09-24/deploy-result-263.json", "project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps271" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps271o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps271b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps271p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-271.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-271.json"), "w", encoding="utf-8"), indent=2)
