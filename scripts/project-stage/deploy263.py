...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-PS263/1.0"
...
NS = 'nadlan-ps263-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-ps263-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    (RB, ['class="nlps-page"', 'id="nlps"', 'id="nlps-t"', 'class="nlbsq"', 'class="nlbslot"', 'id="nlps-view"', 'id="nlps-view-k"', '<h1 id="nl-project-page-title" class="nlps-h1">', 'id="nlpjx-map"', 'class="nl-lead"', 'nlcp-projctx', 'nadlan-project-article', 'רישיון תיווך', 'פרסומת', 'רוצה להופיע כאן?', 'לקבלת תוכניות ומחירים', 'assets/project-stage/bridge.js?ver=1.72.263', 'poster.jpg?ver=1.72.263', 'mapbox-gl-rtl-text/v0.3.0', '<b>תכנון</b>1.2024', 'data-lang="he"', '.nlcp-surr .nlcp-surr__map{display:none', 'body.nl-skin-a .nlsdt .nlsdt-title{color:#1B1A17', '32.10354', 'nlps-ctawrap', 'nlps-hero__cta', 'class="nlpf"', 'class="nlprog"', 'nadlan-ps-faq', 'nadlan-ps-importmap', '<div class="nlps-kicker">', 'class="nlpd"', 'data-nlps-floor="6"', 'data-nlps-floor="13"', 'דירות שנמכרו בריינבו תל אביב, לפי קומה', 'בחרו קומה במגדל, ואחר כך דירה לדוגמה בטבעת הקומה.', '#nla11y{position:fixed;bottom:calc(20px'],
...
def rollback(created_any):
    print("[rollback] restoring .bak263 files")
...
# ---------------------------------------------------------------- main
NEWFILES = set()
FILES = ["inc/project-experience.php", "assets/project-stage/bridge.js"]
...
try:
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("project-stage-2026-09-24/deploy-result-262.json", "project-stage-2026-09-24/deploy-result-261.json", "project-stage-2026-09-24/deploy-result-260.json", "project-stage-2026-09-24/deploy-result-259.json", "project-stage-2026-09-24/deploy-result-258.json", "recommendations-2026-09-24/deploy-result-257.json", "project-stage-2026-09-24/deploy-result-256.json", "project-stage-2026-09-24/deploy-result-255.json", "firgun-2026-09-24/deploy-result-251.json", "tour-calendar-2026-09-24/deploy-result-248.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("ps263" + str(int(time.time()))) if healthy else ["health"]
    if healthy and not bad and not verify_order("ps263o" + str(int(time.time()))):
        bad = ["order"]
    ...
    if bad:
        ...
        bad = verify_pages("ps263b" + str(int(time.time()))) if healthy else ["health"]
        if healthy and not bad and not verify_order("ps263p" + str(int(time.time()))):
            bad = ["order"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-263.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-263.json"), "w", encoding="utf-8"), indent=2)
