# -*- coding: utf-8 -*-
"""Release 1.72.240 (24.9.2026): ready for the register brokers sample.

  inc/directory.php     a broker card without a stated gender (the register gives none) says "תיווך נדל״ן"
  inc/claim-prompt.php  a register broker claims by opening the free site: the link goes to /brokers/?licence=N#join
                        and the join form is prefilled; the claim box takes the site's colours (no blue gradient, no emoji)

Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/broker-sample/deploy240.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "broker-sample-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-BRK240/1.0"
...
NS = 'nadlan-brk240-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/directory.php', 'inc/claim-prompt.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-brk240-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', "מתווכת"], []),
    ("/professionals/%d7%9e%d7%95%d7%99%d7%90%d7%9c-%d7%99%d7%a0%d7%99%d7%91/", ['<h1 class="nlpp-name">', 'class="nlcp"'], ["#2563EB", "🪪"]),
    ("/brokers/?licence=3131540", ['name="licence"', "[name=licence]"], []),
]
...
def rollback(created_any):
    print("[rollback] restoring .bak240 files")
    for rel in ("nadlan-config.php", "inc/directory.php", "inc/claim-prompt.php"):
        print("  ", rel, ops({"restore": rel}, "restore " + rel).get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/directory.php", "inc/claim-prompt.php"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-brk240-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    bad = verify_pages("brk240" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("brk240b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-240.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-240.json"), "w", encoding="utf-8"), indent=2)
