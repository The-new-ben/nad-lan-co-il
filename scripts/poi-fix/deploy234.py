# -*- coding: utf-8 -*-
"""Release 1.72.234 (24.9.2026): project and listing pages never wait on Overpass again.

The Screaming Frog crawl of the whole sitemap measured 14.8 s before the first byte on every uncached project page.
Cause: inc/nearby-poi.php fetched map places (schools, parks, transit) from Overpass inside the page render; one
mirror refused the server, the other hung until the 14 s timeout, and a failed answer was never cached.
v3 queues the fetch in WP-Cron and renders at once; a failure opens a 30-minute breaker.

Ships one plugin file plus a version bump, through a temporary token-gated bridge (wordpress-agent-deploy):
  inc/nearby-poi.php     the fetch moves out of the render path
  nadlan-config.php      version X.Y.Z -> X.Y.(Z+1), patched on the LIVE text (header + constant only)

Before the swap the bridge runs a server-side diagnosis (each Overpass mirror timed from the server, the cron state)
so the cause is proven from the server, not only from outside. Rules kept from deploy233.py: live file must equal git
HEAD, .bak234 siblings on the server + local copies, server lint, OPcache reset + purge, health polled, pages checked,
automatic rollback, bridge deleted and verified 404. The app password is decrypted in-process (DPAPI), never printed.

  python scripts/poi-fix/deploy234.py            diagnose + deploy + verify
  python scripts/poi-fix/deploy234.py --dry      diagnose + checks only, no writes
  python scripts/poi-fix/deploy234.py --rollback restore the .bak234 files
"""
...
QA = os.path.join(REPO, "docs", "qa", "poi-fix-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-POI234/1.0"
...
NS = 'nadlan-poi234-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/nearby-poi.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-poi234-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
SLOW_PROJECTS = [  # measured at 14.8 s TTFB uncached on 24.9.2026, before this release
    "/projects/shikun-binui-sde-dov/",
    "/projects/%D7%94%D7%AA%D7%90%D7%A0%D7%94/",
    "/projects/%D7%9E%D7%9C%D7%9B%D7%99-%D7%A6%D7%93%D7%A7-5-7/",
]
CHECKS = [
    ...
    ("/projects/shikun-binui-sde-dov/", ["<h1", "id=\"nlpjx-map\""], []),
    ("/properties/tzukei-aviv-3-rooms-berlin-for-sale/", ["<article class=\"nlx"], []),
    ("/properties/bialik-rg-4r-demo/", [], []),
]
...
def rollback(created_any):
    print("[rollback] restoring .bak234 files")
    for rel in ("inc/nearby-poi.php", "nadlan-config.php"):
        print("  ", rel, ops({"restore": rel}, "restore " + rel).get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/nearby-poi.php"]
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-poi234-ops-", "x-tmp-had247-ops-")) and x.get("active"):
            print("[sweep] deactivating leftover bridge", x["id"], snip("PUT", f"/{x['id']}/deactivate", {})[0])
    bridge_up()
    d = ops({"diag": 1}, "diag")["diag"]
    print("[diag] server -> Overpass:", json.dumps(d, ensure_ascii=False))
    os.makedirs(QA, exist_ok=True)
    json.dump({"at": time.strftime("%Y-%m-%d %H:%M:%S"), "diag": d}, open(os.path.join(QA, "diag-before.json"), "w", encoding="utf-8"), indent=2)
    before = speed("before")
    if "--rollback" in ARGS:
        rollback(True)
    ...
    bad = verify_pages("poi234" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("poi234b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-234.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-234.json"), "w", encoding="utf-8"), indent=2)
