...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/wa-pill/deploy247.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "wa-pill-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-WA247/1.0"
...
NS = 'nadlan-wa247-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/conversion-cta.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-wa247-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
PILL_NEW = ["background:#0F7A63", "rgba(15,122,99,.30)", '<rect width="64" height="64" rx="14" fill="#F7F6F2"/>']
PILL_OLD = ["background:#25D366", 'fill="#14130F"', "rgba(255,255,255,.85);margin-top:2px"]
CHECKS = [
    ("/", ['class="nlcta-wa"'] + PILL_NEW, PILL_OLD),
    ("/brokers/", ['class="nlcta-wa"', '<h1 class="nlds-pagehead__title">'] + PILL_NEW, PILL_OLD),
    ("/property-value/", ['class="nlcta-wa"', 'class="nlds nlpl"'] + PILL_NEW, PILL_OLD),
    ("/en/", ['class="nlcta-wa"'] + PILL_NEW, PILL_OLD),
    ("/projects/rainbow-tel-aviv/", ["<h1"], []),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">'], PILL_OLD),
    ("/wp-content/plugins/nadlan-config/assets/nlds/nlds.css", ["--nlds-wa-green:#0f7a63"], ["--nlds-wa-green:#25d366"]),
]
...
# ---------------------------------------------------------------- main
FILES = ["inc/conversion-cta.php", "assets/nlds/nlds.css"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    bad = verify_pages("wa247" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("wa247b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-247.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-247.json"), "w", encoding="utf-8"), indent=2)
