...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/broker-video/deploy249.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "broker-video-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-BV249/1.0"
...
NS = 'nadlan-bv249-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/professional-profile.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-bv249-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/", ['class="nlcta-wa"'], []),
    ("/brokers/", ['<h1 class="nlds-pagehead__title">', 'class="nlds-bfeat"', 'class="nlds-procard"'], []),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', "תיאום סיור"], ['class="nlds nlpp-video"']),
    ("/property-value/", ['class="nlds nlpl"'], []),
    ("/projects/rainbow-tel-aviv/", ["<h1"], []),
    ("/wp-content/plugins/nadlan-config/assets/nlds/nlds.css", [":root body .nlds .nlds-video__frame{", ".nlds-video__v--tall{display:block !important"], []),
]
UP = "https://nad-lan.co.il/wp-content/uploads/2026/09/"
VIDEO = {"post": 7833, "value": json.dumps({"tall": UP + "meital-katzir-listings-tall.mp4", "wide": UP + "meital-katzir-listings-wide.mp4",
    "poster_tall": UP + "meital-katzir-listings-tall-poster.jpg", "poster_wide": UP + "meital-katzir-listings-wide-poster.jpg",
    "count": 11, "secs": 51, "date": "2026-09-24"}, ensure_ascii=False)}
...
H1_EXACTLY_ONE = ["/brokers/", "/property-value/", "/projects/rainbow-tel-aviv/"]
PL = None
...
# ---------------------------------------------------------------- main
FILES = ["inc/professional-profile.php", "assets/nlds/nlds.css"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("bv249" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("bv249b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-249.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-249.json"), "w", encoding="utf-8"), indent=2)
    print(f"RELEASE {new} LIVE")

    # the video on Meital's card, only on healthy code; a failure removes the meta and keeps the code
    print("[video] set:", ops({"set_video": VIDEO}, "set video")["set_video"]["post"])
    ops({"purge": 1}, "purge")
    time.sleep(3)
    bad_v = verify_video("bv249v" + str(int(time.time())))
    if bad_v:
        ops({"purge": 1}, "purge again")
        time.sleep(12)
        bad_v = verify_video("bv249w" + str(int(time.time())))
    if bad_v:
        print("[video] FAIL", bad_v, "- removing the meta:", ops({"set_video": {"post": 7833, "value": ""}}, "unset video")["set_video"])
        ops({"purge": 1}, "purge")
    json.dump({"video": json.loads(VIDEO["value"]), "failed": bad_v}, open(os.path.join(QA, "video-249.json"), "w", encoding="utf-8"), indent=2, ensure_ascii=False)
    print("VIDEO", "LIVE" if not bad_v else "OFF")
