...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/tour-calendar/deploy248.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "tour-calendar-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-TC248/1.0"
...
NS = 'nadlan-tc248-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/professional-profile.php', 'inc/brokers-list.php', 'inc/placements.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-tc248-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/", ['class="nlcta-wa"'], []),
    ("/brokers/", ['<h1 class="nlds-pagehead__title">', 'class="nlds-bfeat"', "תיאום סיור", "https://wa.me/972523631582?text=", 'class="nlds-procard"'], ['data-nl-ev="tour"']),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', "תיאום סיור", "https://wa.me/972523631582?text="], ['data-nl-ev="tour"']),
    ("/property-value/", ['class="nlds nlpl"', "התייעצות בוואטסאפ"], ['data-nl-ev="tour"']),
...
# ---------------------------------------------------------------- main
FILES = ["inc/professional-profile.php", "inc/brokers-list.php", "inc/placements.php"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = json.load(open(os.path.join(REPO, "docs", "qa", "placements-2026-09-24", "deploy-result-246.json"), encoding="utf-8"))["files"].get(rel)
        if prev:
            if md5(LIVE[rel]) != prev:
                raise SystemExit(f"FATAL: live {rel} is not what release 246 wrote; diff it before replacing it")
    ...
    bad = verify_pages("tc248" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("tc248b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-248.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-248.json"), "w", encoding="utf-8"), indent=2)
