# -*- coding: utf-8 -*-
"""Release 1.72.241 (24.9.2026): one visible H1 on every regular page (217 had none, per the full-site crawl).

  inc/page-h1.php    NEW: a hero title or an opening h2 that says the page title becomes the H1 (the page's own h2 CSS
                     is copied for h1, so it looks the same); otherwise the page title as an H1 in the design system's
                     PageHeader style, inside the guide wrapper when there is one. Offline test on all 222 pages:
                     26 hero, 42 h2, 106 guide, 48 top, 0 without exactly one H1. Off switch: option nadlan_page_h1 = '0'.
  nadlan-config.php  version bump + 'page-h1' in the module list, both on the LIVE text

Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/page-h1/deploy241.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "page-h1-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-H1-241/1.0"
...
NS = 'nadlan-h1241-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/page-h1.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-h1241-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/", [], ['nlds-pagehead__title']),
    ("/projects/rainbow-tel-aviv/", ["<h1"], ['nlds-pagehead__title']),
    ("/about/", ['<h1'], []),
    ("/investment/modiin-investment-apartment/", ['<h1 class="nlds-pagehead__title">'], []),
    ("/renovation-contractor/", ['nl-h1-from-h2'], []),
    ("/bat-yam/", ['<h1'], []),
    ("/brokers/", ['<h1'], ['nlds-pagehead__title', 'nl-h1-from-h2']),
    ("/mortgage-calculator/", ['<h1'], ['nlds-pagehead__title', 'nl-h1-from-h2']),
]
H1_EXACTLY_ONE = ["/about/", "/investment/modiin-investment-apartment/", "/renovation-contractor/", "/bat-yam/", "/brokers/", "/mortgage-calculator/", "/construction-engineering-guide/"]
...
def rollback(created_any):
    print("[rollback] restoring nadlan-config.php (page-h1.php stays on disk, unused)")
    print("  ", ops({"restore": "nadlan-config.php"}, "restore nadlan-config.php").get("restore"))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/page-h1.php"]
NEWFILES = {"inc/page-h1.php"}
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-h1241-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    anchor = "'placements', 'pro-network' )"
    if text.count(anchor) != 1:
        raise SystemExit(f"FATAL module-list anchor x{text.count(anchor)}")
    text = text.replace(anchor, "'placements', 'pro-network', 'page-h1' )")
    ...
    bad = verify_pages("h1241" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("h1241b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-241.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-241.json"), "w", encoding="utf-8"), indent=2)
