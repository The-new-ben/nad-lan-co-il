# -*- coding: utf-8 -*-
"""Release 1.72.242 (24.9.2026): the shop pages (cart, checkout, account) print their own title; no second H1 there.
...
QA = os.path.join(REPO, "docs", "qa", "page-h1-2026-09-24", "r242")
...
NS = 'nadlan-h1242-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-h1242-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ...
    ("/cart/", ['<h1'], ['nlds-pagehead__title']),
    ("/checkout/", ['<h1'], ['nlds-pagehead__title']),
]
H1_EXACTLY_ONE = ["/cart/", "/checkout/", "/about/", "/investment/modiin-investment-apartment/", "/renovation-contractor/", "/bat-yam/", "/brokers/", "/mortgage-calculator/", "/construction-engineering-guide/"]
...
def rollback(created_any):
    print("[rollback] restoring .bak242 files")
    for rel in ("nadlan-config.php", "inc/page-h1.php"):
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-h1242-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    bad = verify_pages("h1242" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("h1242b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-242.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-242.json"), "w", encoding="utf-8"), indent=2)
