...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/firgun/deploy252.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "firgun-2026-09-24", "r252")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-FG252/1.0"
...
NS = 'nadlan-fg252-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/firgun.php', 'inc/pro-network.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-fg252-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/firgun/", ['<h1 class="nlds-pagehead__title">פרגונים בין אנשי מקצוע</h1>', 'class="nlfg-intro"'], []),
    ("/firgun/?join=1", ['data-fg-f="consent"', "מה קורה עם הפרטים"], []),
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', 'class="nlds nlpp-video"'], []),
    ("/brokers/", ['<h1 class="nlds-pagehead__title">'], []),
    ("/", ['class="nlcta-wa"'], []),
]
VIDEO = None
...
H1_EXACTLY_ONE = ["/brokers/", "/professionals/meital-katzir/", "/firgun/", "/firgun/?join=1"]
PL = None
...
# ---------------------------------------------------------------- main
FILES = ["inc/firgun.php", "inc/pro-network.php"]
NEWFILES = set()
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-fg251-ops-", "x-tmp-fg252-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("firgun-2026-09-24/deploy-result-251.json", "profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    bad = verify_pages("fg252" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("fg252b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-252.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-252.json"), "w", encoding="utf-8"), indent=2)
    print(f"RELEASE {new} LIVE")
    # a wrong private link now answers with its message, not with the host's 404 page
    s3, r3 = req("POST", "/wp-json/nadlan/v1/endorse", body={"token": "a" * 24, "to": 1, "qualities": ["pro"]}, auth=False)
    print("[endorse, wrong link]", s3, (r3 or {}).get("message") if isinstance(r3, dict) else r3)
