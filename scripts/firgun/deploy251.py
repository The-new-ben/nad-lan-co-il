# -*- coding: utf-8 -*-
"""Release 1.72.251 (24.9.2026): the professionals network on screen (owner FOCUS: "פרגונים", the endorsed surface in each
other's pages, a call to join), from the design system (artifact version 7: EndorsementStrip, FirgunForm).
  inc/firgun.php (NEW)          the strip and its given direction on a professional's page; /firgun/ ([nadlan_firgun]):
                                give (?t=), invite, join (?inv=, ?join=1); POST /nadlan/v1/pros/join makes a PENDING card;
                                the waiting endorsement attaches when the card is published; the admin link and notice
  inc/pro-network.php           invitations link to /firgun/; sample cards never come up in the search; gendered labels
  inc/professional-profile.php  prints the strip after the listings video
  assets/nlds/nlds.css          rebuilt from the design system
  nadlan-config.php             'firgun' in the module list (on the LIVE text)
After the code is live: the /firgun/ page is created if missing, the display switch goes on, and a self-test runs inside
one server call: an endorsement Meital -> a register broker is added, both pages' strips are rendered, and the row is
deleted; an invitation is added, the invited page is rendered, and the row is deleted; the give page is rendered for
Meital's token (the token is masked before anything leaves the server); a join request is sent to the real endpoint and
the pending card it made is moved to the trash. Nothing made by the test stays visible.
...
Same safety chain as deploy234.py. The app password is decrypted in-process (DPAPI), never printed.
  python scripts/firgun/deploy251.py [--dry | --rollback]
"""
...
QA = os.path.join(REPO, "docs", "qa", "firgun-2026-09-24")
...
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) NadLan-FG251/1.0"
...
NS = 'nadlan-fg251-' + TOKEN[:8]  # one route per run: a bridge left over from a failed run can never answer for this one
BRIDGE = r'''
add_action( 'rest_api_init', function () {
	register_rest_route( '__NS__/v1', '/apply', array(
		...
		'callback' => function ( $req ) {
			...
			if ( ! empty( $b['fg_page'] ) ) {
				$pg = get_page_by_path( 'firgun' );
				if ( ! $pg ) {
					$pid = wp_insert_post( array( 'post_type' => 'page', 'post_status' => 'publish', 'post_name' => 'firgun', 'post_title' => 'פרגונים בין אנשי מקצוע', 'post_content' => '[nadlan_firgun]' ), true );
					$out['fg_page'] = is_wp_error( $pid ) ? $pid->get_error_message() : array( 'created' => (int) $pid, 'url' => get_permalink( $pid ) );
				} else {
					$out['fg_page'] = array( 'exists' => (int) $pg->ID, 'status' => $pg->post_status, 'content' => $pg->post_content, 'url' => get_permalink( $pg ) );
				}
				update_option( 'nadlan_pro_network_on', '1', false );
				$out['fg_on'] = get_option( 'nadlan_pro_network_on' );
			}
			if ( ! empty( $b['fg_test'] ) ) {
				global $wpdb;
				$t    = function_exists( 'nadlan_endorse_table' ) ? nadlan_endorse_table() : '';
				$from = 7833;
				$tok  = (string) get_post_meta( $from, '_nl_drop_token', true );
				$mask = function ( $h ) use ( $tok ) { return '' !== $tok ? str_replace( $tok, 'TOKEN-MASKED', $h ) : $h; };
				$to   = get_posts( array( 'post_type' => 'nadlan_professional', 'post_status' => 'publish', 'numberposts' => 1, 'fields' => 'ids', 'orderby' => 'ID', 'order' => 'ASC',
					'meta_query' => array( array( 'key' => 'profession', 'value' => 'metavech' ), array( 'key' => 'source', 'value' => 'metavhim' ) ) ) );
				$to   = $to ? (int) $to[0] : 0;
				$res  = array( 'to' => $to, 'token_present' => '' !== $tok );
				if ( function_exists( 'nadlan_endorse_install' ) ) { nadlan_endorse_install(); }
				// 1. an endorsement, both strips, then gone
				$eid = nadlan_endorse_add( $from, $to, 'בדיקה אוטומטית של המערכת, נמחקת מיד.', array( 'pro', 'trust' ) );
				if ( is_wp_error( $eid ) ) { $res['endorse_error'] = $eid->get_error_message(); }
				else {
					$res['strip_to']   = nadlan_fg_strip( $to );
					$res['strip_from'] = nadlan_fg_strip( $from, 'מיטל קציר' );
					$wpdb->delete( $t, array( 'id' => (int) $eid ) );
					$res['endorse_deleted'] = ! (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $t WHERE id = %d", (int) $eid ) );
					$res['strip_to_after'] = nadlan_fg_strip( $to );
				}
				// 2. an invitation, the invited page, then gone
				$iid = nadlan_endorse_add( $from, 0, 'בדיקה אוטומטית של הזמנה, נמחקת מיד.', array( 'avail' ), array( 'name' => 'בדיקה', 'profession' => 'shamai' ) );
				if ( is_wp_error( $iid ) ) { $res['invite_error'] = $iid->get_error_message(); }
				else {
					$itok = (string) $wpdb->get_var( $wpdb->prepare( "SELECT invite_token FROM $t WHERE id = %d", (int) $iid ) );
					$_GET = array( 'inv' => $itok );
					$res['page_invited'] = str_replace( $itok, 'INV-MASKED', nadlan_fg_page() );
					$wpdb->delete( $t, array( 'id' => (int) $iid ) );
					$res['invite_deleted'] = ! (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $t WHERE id = %d", (int) $iid ) );
				}
				// 3. the give page for Meital, token masked; the explainer; the open join form
				$_GET = array( 't' => $tok );
				$res['page_give'] = $mask( nadlan_fg_page() );
				$_GET = array();
				$res['page_plain'] = nadlan_fg_page();
				$_GET = array( 'join' => '1' );
				$res['page_join'] = nadlan_fg_page();
				$_GET = array();
				// 4. a join request through the real endpoint; the pending card goes to the trash
				$day = gmdate( 'Ymd' );
				$before = (int) get_option( 'nlfgj_day_' . $day, 0 );
				$rq = new WP_REST_Request( 'POST', '/nadlan/v1/pros/join' );
				$rq->set_header( 'content-type', 'application/json' );
				$rq->set_body( wp_json_encode( array( 'name' => 'בדיקה אוטומטית', 'profession' => 'lawyer', 'gender' => 'm', 'city' => 'תל אביב יפו', 'phone' => '050-0000001', 'email' => '', 'licence' => '', 'consent' => 1, 'inv' => '', 'company' => '' ) ) );
				$rr = rest_do_request( $rq );
				$res['join_status'] = $rr->get_status();
				$res['join_data']   = $rr->get_data();
				$made = get_posts( array( 'post_type' => 'nadlan_professional', 'post_status' => 'pending', 'numberposts' => 1, 'fields' => 'ids', 'orderby' => 'ID', 'order' => 'DESC',
					'meta_query' => array( array( 'key' => 'source', 'value' => 'firgun_join' ), array( 'key' => 'phone', 'value' => '0500000001' ) ) ) );
				if ( $made ) {
					$mid = (int) $made[0];
					$res['join_card'] = array( 'id' => $mid, 'status' => get_post_status( $mid ), 'title' => get_the_title( $mid ), 'profession' => get_post_meta( $mid, 'profession', true ),
						'gender' => get_post_meta( $mid, 'nl_gender', true ), 'phone' => get_post_meta( $mid, 'phone', true ), 'join' => get_post_meta( $mid, '_nl_join', true ) );
					update_post_meta( $mid, '_nl_test', 'deploy251' );
					$res['join_trashed'] = (bool) wp_trash_post( $mid );
				}
				// the test does not count against today's joins
				update_option( 'nlfgj_day_' . $day, $before, false );
				foreach ( $wpdb->get_col( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE '\_transient\_nlfgj\_%'" ) as $o ) { delete_transient( substr( $o, 11 ) ); }
				$out['fg_test'] = $res;
			}
			...
			if ( ! empty( $b['purge'] ) ) {
				if ( function_exists( 'opcache_invalidate' ) ) { foreach ( array( 'inc/firgun.php', 'inc/pro-network.php', 'inc/professional-profile.php', 'nadlan-config.php' ) as $f ) { @opcache_invalidate( $root . $f, true ); } }
...
def bridge_up():
    global BR
    s, c = snip("POST", "", {"name": f"x-tmp-fg251-ops-{int(time.time())}", "code": BRIDGE, "scope": "global", "active": False})
...
CHECKS = [
    ("/professionals/meital-katzir/", ['<h1 class="nlpp-name">', 'class="nlds nlpp-video"'], ['class="nlds nlpp-network"']),
    ("/brokers/", ['<h1 class="nlds-pagehead__title">', 'class="nlds-bfeat"'], []),
    ("/", ['class="nlcta-wa"'], []),
    ("/projects/rainbow-tel-aviv/", ["<h1"], []),
    ("/wp-content/plugins/nadlan-config/assets/nlds/nlds.css", [":root body .nlds .nlds-pick__item{", ".nlds-endorse__q{"], []),
]
VIDEO = None
FG_PAGES = [
    ("/firgun/", ['<h1 class="nlds-pagehead__title">פרגונים בין אנשי מקצוע</h1>', 'class="nlfg-intro"', "?join=1", 'id="nadlan-firgun-js"'], ['data-fg="give"']),
    ("/firgun/?join=1", ['data-fg="join"', 'data-fg-f="consent"', "מה קורה עם הפרטים"], ['data-fg="give"']),
    ("/firgun/?t=aaaaaaaaaaaaaaaaaaaaaaaa", ["הקישור לא תקין"], ['data-fg="give"']),
]


def verify_firgun(tag):
    bad = []
    for path, need, never in FG_PAGES:
        sep = "&" if "?" in path else "?"
        s2, html = page(path + sep + "nlv=" + tag)
        body = body_of(html)
        miss = [n for n in need if n not in html]
        hit = [n for n in never if n in body]
        h1 = body.count("<h1")
        ok = s2 == 200 and not miss and not hit and h1 == 1 and "Fatal error" not in body
        print(f"[firgun] {'OK ' if ok else 'BAD'} {s2} {path} missing={miss} forbidden={hit} h1={h1}")
        if not ok:
            bad.append(path)
    return bad


def verify_video(tag):
    s2, html = page("/professionals/meital-katzir/?nlv=" + tag)
    body = body_of(html)
    i = body.find('class="nlds nlpp-video"')
    sec = body[i:i + 3000] if i > -1 else ""
    checks = {
        "200": s2 == 200,
        "one section": body.count('class="nlds nlpp-video"') == 1,
        "two videos": sec.count("<video ") == 2,
        "no download before play": sec.count('preload="none"') == 2,
        "no autoplay": "autoplay" not in sec,
        "both posters": "listings-tall-poster.jpg" in sec and "listings-wide-poster.jpg" in sec,
        "meta line": "בסרטון:" in sec and "עודכן" in sec,
        "one h1": body.count("<h1") == 1,
    }
    print("[video]", " | ".join(("OK " if v else "BAD ") + k for k, v in checks.items()))
    return [k for k, v in checks.items() if not v]
H1_EXACTLY_ONE = ["/brokers/", "/professionals/meital-katzir/", "/projects/rainbow-tel-aviv/"]
PL = None

# the placement: Meital (7833), right after the prices section of each page (a page without one gets no card)
PL = {"title": "מיטל קציר: מחשבון השווי ושכונות הצפון והרצליה פיתוח", "meta": {
    "pl_pro": "7833",
    "pl_paths": "\n".join([
        "/property-value/ הרצליה פיתוח",
        "/property-value/luxury-apartments-tel-aviv/ מחיר ושווי",
        "/north-tel-aviv/* מחיר",
        "/ramat-aviv/ מחירים ברמת אביב",
        "/ramat-aviv/einstein/ מחירים וערך",
        "/ramat-aviv/* מחירים",
        "/herzliya-pituach-luxury/ שיקולי בדיקה",
        "/herzliya-pituach-apartment-prices/ מכירה בהרצליה",
    ]),
    "pl_after_h2": "מחיר", "pl_headline": "", "pl_ask": "", "pl_start": "2026-09-24", "pl_end": "", "pl_active": "1"}}
PL_ON = ["/property-value/", "/property-value/luxury-apartments-tel-aviv/", "/north-tel-aviv/", "/north-tel-aviv/old-north/",
         "/ramat-aviv/", "/ramat-aviv/einstein/", "/herzliya-pituach-luxury/", "/herzliya-pituach-apartment-prices/"]
PL_OFF = ["/brokers/", "/ramat-aviv/ramat-aviv-mall/", "/property-value/building-rights-check/", "/new-projects/north-tel-aviv-new-projects/", "/"]


def verify_placement(tag):
    bad = []
    for path in PL_ON + PL_OFF:
        try:
            s2, html = page(path + "?nlv=" + tag)
        except Exception as e:
            print(f"[pl] BAD {path} {e}")
            bad.append(path)
            continue
        body = body_of(html)
        n = body.count('class="nlds nlpl"')
        want = 1 if path in PL_ON else 0
        h1 = body.count("<h1")
        ok = s2 == 200 and n == want and (h1 == 1 or path == "/") and "Fatal error" not in body
        extra = ""
        if n:
            i = body.find('class="nlds nlpl"')
            extra = " | licence in card: %s | label: %s | button: %s" % ("3131540" in body[i:i + 6000], "פרסומת" in body[i:i + 400], "התייעצות בוואטסאפ" in body[i:i + 6000])
        print(f"[pl] {'OK ' if ok else 'BAD'} {s2} {path} cards={n} (want {want}) h1={h1}{extra}")
        if not ok:
            bad.append(path)
    return bad
...
def rollback(created_any):
    print("[rollback] restoring .bak245 files")
    ...
    print("   new icon files removed:", len(NEWFILES))
    ops({"purge": 1}, "purge")


# ---------------------------------------------------------------- main
FILES = ["inc/firgun.php", "inc/pro-network.php", "inc/professional-profile.php", "assets/nlds/nlds.css"]
NEWFILES = {"inc/firgun.php"}
pass
...
try:
    ...
    for x in (lst if s == 200 and isinstance(lst, list) else []):
        if str(x.get("name", "")).startswith(("x-tmp-bl244-ops-", "x-tmp-bl245-ops-", "x-tmp-pl246-ops-", "x-tmp-wa247-ops-", "x-tmp-tc248-ops-", "x-tmp-bv249-ops-", "x-tmp-pe250-ops-", "x-tmp-fg251-ops-", "x-tmp-had247-ops-")) and x.get("active"):
    ...
    for rel in FILES:
        ...
        prev = None
        for res in ("profile-emoji-2026-09-24/deploy-result-250.json", "broker-video-2026-09-24/deploy-result-249.json", "tour-calendar-2026-09-24/deploy-result-248.json", "wa-pill-2026-09-24/deploy-result-247.json", "placements-2026-09-24/deploy-result-246.json"):
    ...
    anchor = "'design-system', 'brokers-list' ) as $nadlan_mod"
    if text.count(anchor) != 1:
        raise SystemExit("FATAL module anchor x%d" % text.count(anchor))
    text = text.replace(anchor, "'design-system', 'brokers-list', 'firgun' ) as $nadlan_mod")
    ...
    bad = verify_pages("fg251" + str(int(time.time()))) if healthy else ["health"]
    ...
    if bad:
        ...
        bad = verify_pages("fg251b" + str(int(time.time()))) if healthy else ["health"]
    if bad:
    ...
    json.dump({"released": new, "from": old, "at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        open(os.path.join(QA, "deploy-result-251.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"before": before, "after": after}, open(os.path.join(QA, "speed-251.json"), "w", encoding="utf-8"), indent=2)
    print(f"RELEASE {new} LIVE")

    # the /firgun/ page and the switch, then the self-test (nothing it makes stays visible)
    fp = ops({"fg_page": 1}, "firgun page")
    print("[firgun] page:", {k: v for k, v in fp["fg_page"].items() if k != "content"}, "| switch:", fp["fg_on"])
    ops({"purge": 1}, "purge")
    time.sleep(3)
    ft = ops({"fg_test": 1}, "firgun self-test")["fg_test"]
    json.dump(ft, open(os.path.join(QA, "selftest-251.json"), "w", encoding="utf-8"), indent=1, ensure_ascii=False)
    checks = {
        "endorsement added": "endorse_error" not in ft,
        "strip on the receiver": 'class="nlds-endorse__item"' in ft.get("strip_to", "") and "מקצועיות" in ft.get("strip_to", ""),
        "given on Meital": "מיטל ממליצה על" in ft.get("strip_from", "") or "ההמלצות של" in ft.get("strip_from", ""),
        "endorsement deleted": ft.get("endorse_deleted") is True and ft.get("strip_to_after", "x") == "",
        "invited page": "פרגון מחכה לך" in ft.get("page_invited", "") and ft.get("invite_deleted") is True,
        "give page": 'data-fg="give"' in ft.get("page_give", "") and "TOKEN-MASKED" in ft.get("page_give", ""),
        "plain page": 'class="nlfg-intro"' in ft.get("page_plain", ""),
        "join page": 'data-fg="join"' in ft.get("page_join", ""),
        "join endpoint": ft.get("join_status") == 200 and (ft.get("join_data") or {}).get("ok") == 1,
        "pending card": (ft.get("join_card") or {}).get("status") == "pending" and ft.get("join_trashed") is True,
    }
    print("[selftest]", " | ".join(("OK " if v else "BAD ") + k for k, v in checks.items()))
    ops({"purge": 1}, "purge")
    time.sleep(3)
    bad_f = verify_firgun("fg251p" + str(int(time.time())))
    if bad_f:
        ops({"purge": 1}, "purge again")
        time.sleep(12)
        bad_f = verify_firgun("fg251q" + str(int(time.time())))
    failed = [k for k, v in checks.items() if not v] + bad_f
    print("FIRGUN", "LIVE" if not failed else "CHECK: " + ", ".join(failed))
