...
function genScene(spec) {
  ...
  addBox(G.sea, -300, -0.5, 0, 520, 1.0, 1000, 0);
  addBox(G.beach, -58, -0.32, 0, 44, 0.7, 1000, 0);
  addBox(G.land, 250, -0.5, 0, 560, 1.0, 1000, 0);
  addBox(G.road, -30, 0.06, 0, 8, 0.12, 1000, 0);
  for (let r = 1; r <= 4; r++) addBox(G.road, 30 + r * 78, 0.05, 0, 4, 0.1, 1000, 0);
  for (let r = -3; r <= 3; r++) addBox(G.road, 170, 0.05, r * 150, 460, 0.1, 5, 0);
  ...
  for (let n = 0, tr = 0; n < spec.ctxCount && tr < spec.ctxCount * 14; tr++) {
    const x = 40 + rng() * 250, z = (rng() * 2 - 1) * 420;
...
const SPECS = {
  ashira:  { floors: 20, floorH: 3.05, podiumFloors: 2, fpX: 24, fpZ: 19, twist: 0,   taperTop: 0.90, seed: 11, ctxCount: 30,
             landmark: { x: -6,  z: -165, h: 58 }, neighbors: [{ x: 150, z: -170, floors: 16, rot: 8 }, { x: 185, z: 150, floors: 14, rot: -6 }],
             units: [ { id: 'ashira-18-west', label: '18W', floor: 18, bearing: 270 }, { id: 'ashira-14-city', label: '14C', floor: 14, bearing: 90 },
                      { id: 'ashira-10-corner', label: '10P', floor: 10, bearing: 225 }, { id: 'ashira-07-east', label: '7A', floor: 7, bearing: 90 },
                      { id: 'ashira-04-garden', label: '4G', floor: 4, bearing: 270 } ] },
  rainbow: { floors: 40, floorH: 3.05, podiumFloors: 3, fpX: 24, fpZ: 20, twist: 0.7, taperTop: 0.84, seed: 23, ctxCount: 28,
             landmark: { x: -10, z: -180, h: 58 }, neighbors: [{ x: 165, z: 160, floors: 22, rot: 0 }, { x: 185, z: -150, floors: 20, rot: 6 }],
             units: [ { id: 'rainbow-31', label: '31W', floor: 31, bearing: 270 }, { id: 'rainbow-24', label: '24N', floor: 24, bearing: 315 },
                      { id: 'rainbow-16', label: '16W', floor: 16, bearing: 270 }, { id: 'rainbow-08', label: '8S', floor: 8, bearing: 225 } ] },
  dimri:   { floors: 24, floorH: 3.10, podiumFloors: 2, fpX: 28, fpZ: 22, twist: 0,   taperTop: 0.93, seed: 31, ctxCount: 26,
             landmark: { x: -12, z: -175, h: 58 }, neighbors: [{ x: 170, z: 150, floors: 20, rot: 0 }, { x: 185, z: -140, floors: 18, rot: 0 }],
             units: [ { id: 'dimri-22', label: '22W', floor: 22, bearing: 270 }, { id: 'dimri-15', label: '15S', floor: 15, bearing: 225 },
                      { id: 'dimri-09', label: '9N', floor: 9, bearing: 315 } ] },
};
