(function () {
	...
	var panelCopy = {
		plan: 'כאן תוצג תוכנית הדירה לאחר העלאת תוכנית מכר מאושרת. בגרסת ההמחשה מוצג מקום שמור כדי לשמור על מבנה החוויה.',
		view: 'כאן יופיע מבט מהדירה לפי קומה וכיוון, עם חיבור עתידי למפה או לתצוגת תלת ממד של הסביבה.',
		tour: 'כאן יופיע סיור פנים או סרטון שיווקי כאשר היזם יעלה מדיה מאושרת לפרויקט.',
		contact: 'השאירו פרטים עם הדירה שנבחרה, ונציג יחזור עם מידע עדכני וזמינות.'
	};
	...
	function submit(root, form, event) {
		...
		text(ok, 'הפנייה נשמרה בהמחשה עם פרטי ' + unitTitle + '. בחיבור החי היא תישלח לערוץ הפנייה של הפרויקט.');
		if (ok) {
			ok.hidden = false;
		}
	}
