...
For the current Einstein payload, treat `create_or_replace_exact_private_sandbox` as a legacy,
misnamed metadata value. The guarded driver implements create-only and aborts when the exact slug
already exists; it does not provide replace/update authority for this stage.
