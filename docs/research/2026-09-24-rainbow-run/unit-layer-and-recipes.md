...
| 6 | **The non-affiliation notice opens the article section** (the owner's order of 29.8.2026: out of the snippet zone, still in the body and naming the developer; `inc/legal-notice.php` places it and `tools/source_audit.py` checks it). The Einstein order and FSR put it right after the lead, but the owner's later order wins (1.72.259 followed the old order and the audit caught it on 25.9). The full disclaimer block goes last (row 28) | owner 29.8.2026, EIN `page_order`[2], FSR §9.2, V1 row 30 | VS position (the audit's notice light) |
...
| 6 notice | `nl-projnotice` "אתר עצמאי" opens the article section, after the stage, the map and the prices | done (the owner's 29.8 order; the audit checks it) |
