...
Date: 2026-06-23
Branch: `codex/showroom-1691-clean-release`
...
This clean release branch contains only the plugin runtime files, the 1.69.1 ZIP and manifest, the showroom visual QA harness, and this screenshot evidence. It does not include the Lovable prototype, strategy folders, or the earlier 1.69.0 ZIP.
...
- The active showroom CSS now loads Frank Ruhl Libre and Heebo through Google Fonts because the self-hosted theme font files are not present in this clean plugin release.
...
- On desktop, the model still renders small inside a large stage. This release fixes the dark-skin and stacked-CSS problem, but it is not yet the final contractor-pitch layout.
...
- Inline JavaScript extracted from `project-3d.php`: passed, `inline JS ok 70195`.
...
- Visual harness: passed 4 of 4 viewports after replacing the live 1.68.2 showroom CSS with the local 1.69.1 preview CSS.
