...
| Mobile model space | All six live stages measured about 368 x 490.7 px at 390 px; body width did not overflow | `.nlfs__protected-stage` reserves `max(430px, 68svh)` on mobile and clips overlays; offline QA passed four viewports with the model center hit-testable | Directly implements the owner's “building is the jewel” rule | Same geometry must pass in real WordPress with header, cookie/WhatsApp chrome and browser safe areas |
...
| Interior and facilities | Legacy engine can lazy-load a tour, panoramas and generated first-person room geometry when its fields exist | Offline QA proves four selectable concept scenes, step/turn/door/light simulation and model-anchor entry | Visually teases the user and gives facilities an explicit selectable lane | It is not a 360 tour, spatial walk, official plan or facility specification; upgrade through a scene-provider interface when contractor assets arrive |
...
| Integration and rollback | Legacy renderer is already public and fleet-wide | Loader and dispatcher seams now exist in the working tree; PHP fixture passes; public release remains false and the v3 branch fails closed | Versioned selection protects unselected projects | Integration plan still says “not applied”, no private WordPress clone/readback exists, and canonical post `4867` must remain untouched until explicit release |
| Reuse | One large legacy engine handles many pages, with project-specific data and historical additions | v3 splits shared renderer/viewer/playground from governed project contracts | Better direction for scalable flagship packages | Current active recipe references missing canonical playbook/generator files and its `<1 MB` model rule conflicts with the new 30,000-triangle flagship requirement |
...
1. Reconcile the integration note with the working-tree seam and run the full PHP/integration suite after the branch freezes.
