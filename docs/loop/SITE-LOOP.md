...
| X1 | Small defects found on the way (Linear HAD-287, a checklist): Rainbow's view labels in English; a second map entry in the surroundings band; the faint "חדש - סיור תלת ממדי חי" box; the phone progress label crowding; the stage poster for the new aspect; Dimri Yama's 79-character title; map chips with emoji; Aurelia "חסרות קואורדינטות למפה"; /earth/ layer 401; Somail pins and tour positions; catalog-plus and tools/deals not in git | **next** | the accessibility button moved to the bottom corner in 1.72.261 (owner's word) |
