...
| R6 | Rainbow cinematic pass: a detailed tower, sun and soft shadows, atmosphere, the opening flight, first picture < 4 s on a mid Android | **part 1 done 25.9 (1.72.272)**; part 2 **next** | hauzd-teardown.md §5 items 2, 8, 9, 10. **Part 1 (DS v29, Linear HAD-296):** the first picture was made by the stage's script after every other script (9.1 s on a mid Android, measured with scripts/project-stage/firstpic.py: 4x CPU, Slow 4G) and the page's layout CSS came from the footer (the top unarranged until the whole page arrived); now the poster is in the page's HTML (a 38 KB phone file in srcset, high priority), adopted by the script, and the CSS is at the end of the head: live LCP = the poster at 3.33-3.43 s, the live 3D at 9.5-10 s; pins never cover the stage's hint, light switch or caption. **Part 2:** the opening flight (a 4-6 s eased push-in, reduced-motion aware), a real sun path with soft shadows (sunrise to night, lit windows as illustration), atmosphere (haze on the city), the tower's detail (mullions, railings); keep the weight budget (first view ≤ 10 MB) |
...
## Done in this loop
- 25.9 morning: R6 part 1 (1.72.272: the first picture in 3.4 s on a mid Android, was 9.1 s; the page CSS in the head;
  pins clear of the stage's chrome; DS v29; HAD-296).
