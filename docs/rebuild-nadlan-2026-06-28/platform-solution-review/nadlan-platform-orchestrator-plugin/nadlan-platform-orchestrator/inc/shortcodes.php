...
function nlpo_catalog_shortcode( $atts, $content = null, $tag = '' ) {
	...
	$subtitle = $a['subtitle'] !== '' ? $a['subtitle'] : __( 'בחירת פרויקטים עם דירות, סביבה, אומדן ותוכן קנייה מלא. התוכן והלידים נשארים במערכת הקיימת.', 'nadlan-platform-orchestrator' );
	ob_start();
	?>
...
function nlpo_showroom_shortcode( $atts ) {
	...
	return '<div class="nlp-interior"><div class="nlp-interior-stage"><p class="nlp-interior-placeholder">' . esc_html__( 'מנוע הפרויקט יופעל כאשר nadlan-config יהיה פעיל.', 'nadlan-platform-orchestrator' ) . '</p></div></div>';
}
...
function nlpo_interior_shortcode( $atts ) {
	...
	<section class="nlp-interior" data-nlpo-interior>
		...
		<div class="nlp-interior-stage" data-nlpo-interior-stage>
			<?php if ( $tour ) : ?>
				<p class="nlp-interior-placeholder"><?php esc_html_e( 'סיור פנים זמין. לחיצה תטען אותו במקום זה.', 'nadlan-platform-orchestrator' ); ?></p>
			<?php elseif ( ! empty( $panos ) ) : ?>
				<p class="nlp-interior-placeholder"><?php esc_html_e( 'תמונות פנים פנורמיות זמינות במערכת. הטעינה נעשית לפי בחירת המשתמש כדי לשמור על מהירות.', 'nadlan-platform-orchestrator' ); ?></p>
			<?php else : ?>
