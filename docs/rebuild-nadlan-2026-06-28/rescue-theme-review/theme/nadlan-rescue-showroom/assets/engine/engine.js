...
(function () {
  ...
  /* standalone-bundle asset resolver: maps code-referenced asset paths to inlined
     blob URLs when the page is exported via super_inline_html (ext-resource-dependency
     meta tags + window.__resources). No-op in the normal multi-file build. */
  var RESMAP = {};
  (function () {
    if (typeof window !== "undefined" && window.__resources) {
      var ms = document.querySelectorAll('meta[name="ext-resource-dependency"]');
      for (var i = 0; i < ms.length; i++) {
        var id = ms[i].getAttribute("data-resource-id"), c = ms[i].getAttribute("content");
        if (id && c && window.__resources[id]) RESMAP[c] = window.__resources[id];
      }
    }
  })();
  function rAsset(p) {
    if (!p) return p;
    if (RESMAP[p]) return RESMAP[p];
    if (/^(https?:)?\/\//.test(p) || p.charAt(0) === "/" || p.indexOf("data:") === 0) return p;
    var base = (window.NADLAN_ASSET_BASE || "").replace(/\/$/, "");
  ...
  function dirLabel(d) { var known = {west:1,east:1,north:1,south:1,"south-west":1,"north-west":1,"south-east":1,"north-east":1}; return known[d] ? t("dir_" + d) : (d || ""); }
  ...
  function header() {
    var nav = state.page === "home"
      ...
      : '<div class="nl-nav__links"><a href="/">' + esc(t("nav_projects")) + '</a><a href="#world">' + esc(t("nav_areas")) + '</a><a href="#inquiry">' + esc(t("hero_cta_primary")) + "</a></div>";
    return '<header class="nl-header"><div class="nl-wrap nl-header__row">' +
      '<a class="nl-brand" href="/"><span class="nl-brand__mark">N</span><span><span class="nl-brand__name">' + esc(t("brand")) + '</span> <span class="nl-brand__sub">' + esc(t("brand_sub")) + "</span></span></a>" +
  ...
  function hero() {
    ...
    return '<div class="nl-hero">' +
      "<div>" +
        ...
        "</div>" +
      "</div>" +
      '<div class="nl-hero__media"><img src="' + esc(rAsset(p.model_poster)) + '" alt="' + esc(projName()) + '" loading="eager">' + (SR.config.demo ? '<span class="nl-badge nl-badge--demo nl-hero__badge">' + esc(t("demo_badge")) + "</span>" : "") + "</div>" +
    "</div>";
  }
  ...
  function theater() {
    ...
    var facadeBg = p.facade_image ? ' style="background-image:url(' + esc(rAsset(p.facade_image)) + ')"' : "";
    ...
    return '<div class="nl-theater">' +
      ...
      '<div class="nl-stagewrap">' +
        '<model-viewer id="nl-mv" class="nl-stage" src="' + esc(rAsset(p.model_glb)) + '" loading="eager" reveal="auto" camera-controls auto-rotate auto-rotate-delay="800" rotation-per-second="14deg" interaction-prompt="none" environment-image="neutral" exposure="1.02" shadow-intensity="0.55" shadow-softness="1" camera-orbit="' + esc(p.default_orbit) + '" camera-target="' + esc(p.default_target) + '" min-camera-orbit="auto 48deg auto" max-camera-orbit="auto 86deg auto" min-field-of-view="16deg" max-field-of-view="44deg" touch-action="pan-y">' + hots + "</model-viewer>" +
        '<div class="nl-poster" id="nl-poster" style="background-image:url(' + esc(rAsset(p.model_poster)) + ')"></div>' +
  ...
  function tabPane(u) {
    if (state.tab === "plan") return u.plan ? '<img src="' + esc(rAsset(u.plan)) + '" alt="' + esc(t("tab_plan")) + '">' : "<p>" + esc(t("plan_coming")) + "</p>";
    if (state.tab === "view") return u.interior_url ? '<img src="' + esc(rAsset(u.interior_url)) + '" alt="">' : "<p>" + esc(t("view_coming")) + "</p>";
  ...
  function world() {
    ...
    var nearby = SR.order.filter(function (k) { return k !== state.projectKey; }).map(function (k) {
      ...
      return '<a class="nl-card" href="' + esc((SR.projects[k] && SR.projects[k].url) || ("/projects/" + k + "/")) + '" style="text-decoration:none;display:block"><div class="ic">' + svg("cube", 18) + '</div><h4>' + esc(t(pr.name_key)) + '</h4><p>' + esc((pr.content[state.lang] || pr.content.en || {}).tagline || "") + "</p></a>";
    }).join("");
  ...
  function media() {
    ...
    (p.gallery || []).forEach(function (g) { tiles += '<div class="nl-card" style="padding:0;aspect-ratio:4/3;background:#ddd center/cover url(' + esc(rAsset(g)) + ')"></div>"'; });
  ...
  /* footer */
  function footer() {
    var projLinks = SR.order.map(function (k) { return '<li><a href="' + esc((SR.projects[k] && SR.projects[k].url) || ("/projects/" + k + "/")) + '">' + esc(t(SR.projects[k].name_key)) + "</a></li>"; }).join("");
    ...
    return '<footer class="nl-footer"><div class="nl-wrap"><div class="nl-footer__row">' +
      '<div><a class="nl-brand" href="/"><span class="nl-brand__mark">N</span><span class="nl-brand__name" style="color:#efe7d6">' + esc(t("brand")) + '</span></a><p style="color:#b8b1a2;font-size:14px;margin-top:12px;max-width:34ch">' + esc(t("footer_tagline")) + "</p></div>" +
