...
// Inject Mobile Buying Experience Action Rail
add_action("wp_footer", function() {
    if (!is_singular("nadlan_project")) return;
    
    $phone = get_option("nadlan_contact_phone", "054-0000000");
    $wa_num = preg_replace("/[^0-9]/", "", $phone);
    if (strpos($wa_num, "0") === 0) $wa_num = "972" . substr($wa_num, 1);
    
    $wa_url = "https://wa.me/" . $wa_num . "?text=" . urlencode("????, ???? ?????? ?? ???????: " . get_the_title());
    ?>
    <div class="nl-mobile-action-rail" style="display: none;">
        ...
        <a href="<?php echo esc_url($wa_url); ?>" target="_blank" class="nl-rail-btn nl-rail-wa">
        ...
        <a href="#offer" class="nl-rail-btn nl-rail-offer" onclick="document.querySelector(\".nlp3d-form\") ? document.querySelector(\".nlp3d-form\").scrollIntoView({behavior:\"smooth\"}) : null; return false;">
